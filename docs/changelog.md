...
- Turning a session provider on or off, or adding or removing an alternate
  home, on the Settings page now takes effect right away. You no longer need to
  restart the daemon. Sessions already in a newly enabled provider or home are
  synced immediately, and new sessions are picked up as they are written.
  Separate `push --watch` processes still need a restart.
