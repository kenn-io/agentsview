...
- Codex sessions tagged `thread_source=roborev` (from `codex exec
    --thread-source roborev`) show as automated. Restart or run
    `agentsview sync` so existing tagged sessions reparse.
