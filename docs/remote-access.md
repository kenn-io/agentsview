...
## title: Remote Access description: Access AgentsView from other devices on your network
...
!!! note Older configs may still use `remote_access = true`. AgentsView still
reads that legacy key, but new setups should use `require_auth = true`.
...
`403 Forbidden` with a response body that explains the rejected host, the
allowed set, and the `--public-url` fix. The Settings page shows the same
actionable message instead of prompting for an auth token. A short breadcrumb is
also written to the server debug log.
...
!!! warning "Enable auth first" Managed Caddy exposes a public endpoint. Set
`require_auth = true` in `~/.agentsview/config.toml` before starting the server,
or the proxy will front an unauthenticated API. See [Quick Setup](#quick-setup).
