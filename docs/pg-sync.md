...
## title: PostgreSQL Sync description: Share sessions across machines with PostgreSQL push sync, an auto-push service, and a read-only server
...
PostgreSQL. No local SQLite, file watching, or uploads — just the viewer.
...
interval acts as a floor: even if filesystem events are missed or a root cannot
be watched because of OS watch limits, the next interval push catches up.
...
- The watcher uses the same `[pg]` config, machine name, project filters,
...
[`agentsview projects`](/commands/#agentsview-projects) to list available
project names.

!!! warning Filtered pushes do not advance the global push watermark because it
covers all projects. This means the query window for subsequent filtered pushes
grows over time. Run an unfiltered `pg push` (or `pg push --all-projects`)
...
flag is required. The [`agentsview secrets`](/commands/#agentsview-secrets)
...
the [`has-secret`](/session-api/#agentsview-session-list) list filter).
...
`install` requires a literal `[pg].url` in `~/.agentsview/config.toml`. It
intentionally rejects `AGENTSVIEW_PG_URL` and environment-expanded URLs such as
`${PG_PASSWORD}` because background services do not inherit your interactive
shell environment. Other session-directory environment variables are not copied
into the unit either; put persistent settings in `config.toml` before
installing.
...
The server is read-only — uploads, file watching, and local sync are all
disabled. Sessions from all machines appear in a single unified view. The same
deployment also serves the analytics dashboard and the Usage page from
...
!!! warning Query-parameter tokens can leak through server logs, browser
history, and Referer headers. Prefer the `Authorization` header for all non-SSE
requests, and use TLS (via managed Caddy or an external reverse proxy) to
...
!!! warning "Enable auth before exposing `pg serve`" Only the loopback example
below is safe without auth. Every other example binds off `127.0.0.1` or fronts
a public proxy, so set `require_auth = true` in `~/.agentsview/config.toml`
before starting the server. The same bearer-token mechanism described in
...
!!! warning The `url` field is required for all `pg` commands. If it contains
credentials, ensure `config.toml` has restricted file permissions (`0600`).
...
PostgreSQL settings can also be configured via environment variables, which
...
- **Permanent deletes not propagated** — sessions removed via `agentsview prune`
  are not deleted from PostgreSQL because the local rows no longer exist at push
  time. Use a direct SQL DELETE to clean up PostgreSQL if needed. Soft-deleted
