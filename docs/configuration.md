...
| `[friction]`                        | Friction Log review, on by default: `enabled`, `timezone` (IANA; `AGENTSVIEW_FRICTION_TZ` overrides), `backfill_days` (default 7), `seat_patterns` (session path globs with one `{seat}` capture)                                                                                                                                                                                                                                                                                |
| `[friction.nanoclaw]`               | NanoClaw personas, off until `data_dir` is set: `data_dir`, `db` (default `<data_dir>/v2.db`), `include`, `exclude` (agent id, persona or folder; exclude wins; unresolved agents fail closed when either is set)                                                                                                                                                                                                                                                                |
