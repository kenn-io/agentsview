...
| `[vector]`                          | Opt-in semantic-search index; endpoint and model settings live in `[vector.embeddings]`, embedding schedule in `[vector.embed]` — see [Semantic Search](/semantic-search/#enabling-vector) for every key |
