...
The local artifact repository lives at `$AGENTSVIEW_DATA_DIR/artifacts`. It is
an AgentsView-owned Docbank vault with a private catalog, blob, and pack layout.
Do not inspect, edit, copy, or synchronize files inside this directory, and do
not use it as a folder-sync target. A single writable AgentsView daemon owns the
vault; maintenance commands use that daemon when it is available or acquire
exclusive local ownership before opening the vault directly.
...
file with a `.corrupt` suffix so a valid holder can replace it. Read and export
paths fall back to the previous valid checkpoint, and metadata events whose
timestamps do not parse are rejected at write time and skipped at replay.
