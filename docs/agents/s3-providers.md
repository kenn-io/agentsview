...
| Cursor | Harvest `<project>/<id>.{jsonl,txt}` plus local `agent-transcripts` layouts; decode encoded project dirs only for the local layout; prefer `.jsonl` then nested over flat |
