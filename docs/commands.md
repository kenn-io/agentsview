...
Recurring artifact exchange publishes only SQLite sessions marked as changed.
Received artifacts are imported by exact changed reference; incomplete
checkpoint or metadata work is retained durably and retried in bounded batches
after dependencies arrive and on startup.
