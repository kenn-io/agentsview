...
## title: DuckDB Mirror description: Mirror the local SQLite archive into DuckDB and serve it locally or over the Quack remote protocol
...
truth for ingestion; the mirror is populated by an explicit push, the same
one-way model as [PostgreSQL sync](/pg-sync/).
...
!!! warning "Experimental" The DuckDB backend is new in 0.33.0, and Quack is a
beta DuckDB core extension. Expect rough edges, and treat the Quack remote path
as suitable for trusted networks only.
...
- A token is required; if you don't pass one, a token is generated and printed
  so clients can authenticate.
- Binding to a non-loopback address requires the explicit `--allow-insecure`
  flag. Quack speaks plain HTTP, so put it behind TLS, a VPN, or an SSH tunnel
...
- **One-way mirror** — data flows from SQLite to DuckDB only, via explicit
  `duckdb push`. There is no watch mode yet; rerun the push to refresh the
  mirror.
