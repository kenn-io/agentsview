...
is on the client's `PATH`. The hook invokes that binary and trusts the
client's own plugin-root variable over any generic override, so review
project-level settings that alter either before trusting a repository. The
default target is the local archive owner. Select
...
any local edits. Native client uninstall owns only this package and leaves the
AgentsView archive intact.
...
under the MIT License. The generated files retain the full notice and describe
the AgentsView-specific adaptation.
