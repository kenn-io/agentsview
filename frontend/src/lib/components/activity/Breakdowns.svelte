<script lang="ts">
  ...
  const panels = $derived.by((): Panel[] => [
    { title: $_("activity.project"), rows: byProject },
    { title: $_("activity.model"), rows: byModel },
    { title: $_("activity.agent"), rows: byAgent },
  ]);
  ...
  function showTip(e: MouseEvent, row: ActivityKeyMinutes, total: number) {
    ...
    const unit = metric === "cost" ? "" : $_("activity.minUnit");
    const split = $_("activity.intAutoSplit", { values: { int: fmtSeg(interactiveValue(row)), auto: fmtSeg(automatedValue(row)) } });
    tooltip = {
...
<div class="breakdowns">
  <div class="breakdowns-header">
    <h3 class="breakdowns-title">{$_("activity.breakdown")}</h3>
    <div class="header-right">
      ...
      <div class="metric-toggle" role="group" aria-label={$_("activity.breakdownMetric")}>
      <button
        type="button"
        class="metric-btn"
  ...
  <div class="breakdown-grid">
    {#each panels as panel (panel.title)}
      ...
      <div class="breakdown-panel">
        ...
        {:else}
          <div class="empty">{$_("activity.noneLower")}</div>
        {/if}
      </div>
    {/each}
  </div>
