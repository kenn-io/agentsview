...
describe("PeersPage", () => {
  ...
  it("clears a stale conflict banner when refresh fails", async () => {
    mocks.getPeers
      .mockResolvedValueOnce(page([peer("peer-one")], undefined, 1))
      .mockRejectedValueOnce(new Error("artifact store unavailable"));
    render(PeersPage);

    await screen.findByText("1 metadata conflict");
    await fireEvent.click(
      screen.getByRole("button", { name: "Refresh peer status" }),
    );

    await screen.findByText("Artifact sync is not available on this server.");
    expect(screen.queryByText("1 metadata conflict")).toBeNull();
  });
});
