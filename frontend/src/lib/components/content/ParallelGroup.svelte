...
<script lang="ts">
  ...
  let upperBoundLabel = $derived.by(() => {
    ...
    return t(m.parallel_group_each_duration, {
...
<div class="parallel-group">
  <div class="pg-header">
    <span class="pg-label">{t(m.parallel_group_label)}</span>
    <span class="pg-count">{t(m.parallel_group_call_count, {
      count: String(toolCalls.length),
    })}</span>
    <span class="pg-spacer"></span>
    {#if isRunning}
      <span class="pg-running">{t(m.parallel_group_running)}</span>
