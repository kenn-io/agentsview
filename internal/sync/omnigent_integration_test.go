...
func TestSyncSingleSessionOmnigentIdentityMigrationHonorsLegacyDeletionState(
	t *testing.T,
) {
	if testing.Short() {
		t.Skip("skipping integration test")
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			const (
				legacyID    = "omnigent:conv_0000"
				qualifiedID = "omnigent:7:conv_0000"
			)
			qualified, err := archive.GetSession(t.Context(), qualifiedID)
			require.NoError(t, err)
			require.NotNil(t, qualified)
			legacyPath := archive.GetSessionFilePath(qualifiedID)
			require.NotEmpty(t, legacyPath)
			require.NoError(t, archive.UpsertSession(db.Session{
				ID: legacyID, Agent: string(parser.AgentOmnigent),
				Machine: "local", Project: "project", FilePath: &legacyPath,
			}))
			tc.deleteOld(t, archive, legacyID)

			require.NoError(t, engine.SyncSingleSession(qualifiedID))

			qualified, err = archive.GetSession(t.Context(), qualifiedID)
			require.NoError(t, err)
			assert.Nil(t, qualified,
				"single-session sync must not revive a replacement over legacy deletion")
			tc.assertOld(t, archive, legacyID)
		})
	}
}

func TestSyncSingleSessionOmnigentIdentityMigrationKeepsPrePolicyCwdAdmission(
	t *testing.T,
) {
	if testing.Short() {
		t.Skip("skipping integration test")
	...
	dbPath := writeOmnigentSyncDB(t, root, 1)
	archive := dbtest.OpenTestDB(t)
	initial := sync.NewEngine(archive, sync.EngineConfig{
		AgentDirs: map[parser.AgentType][]string{
			parser.AgentOmnigent: {root},
		},
		Machine: "local",
	})
	initial.SyncPaths([]string{dbPath})
	migrateOmnigentSyncDBToSplit(t, dbPath, 7, "conv_0000")
	initial.SyncPaths([]string{dbPath})
	initial.Close()
	...
	require.NoError(t, archive.SoftDeleteSession(legacyID))
	...
	require.NoError(t, filtered.SyncSingleSession(qualifiedID))
	qualified, err = archive.GetSession(t.Context(), qualifiedID)
	require.NoError(t, err)
	assert.Nil(t, qualified,
		"an in-scope single-session parse must retain cleanup admission")
	assert.True(t, archive.IsSessionTrashed(legacyID))
}
