...
func TestFlushSurvivesRestart(t *testing.T) {
	...
	require.NoError(t, cache.Flush(database))
	cache.Clear("claude:/cleared.jsonl")
	require.NoError(t, cache.Flush(database))

	var restarted Cache
	require.NoError(t, restarted.Load(database))
...
func TestFlushWritesToEachTarget(t *testing.T) {
	...
	require.NoError(t, cache.Flush(replacement))
	require.NoError(t, cache.Flush(live))

	persisted, err := live.LoadSourceFailures()
	require.NoError(t, err)
