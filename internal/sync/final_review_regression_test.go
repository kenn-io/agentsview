...
func TestIdentityMigrationRetainsLegacyUntilReplacementWriteSucceeds(
	t *testing.T,
) {
	const (
		agent      parser.AgentType = "identity-write-order"
		previousID string           = "identity:legacy"
		currentID  string           = "identity:current"
	)
	database := openTestDB(t)
	sourcePath := filepath.Join(t.TempDir(), "shared.db#current")
	require.NoError(t, database.UpsertSession(db.Session{
		ID: previousID, Agent: string(agent), Machine: "local",
		Project: "project", FilePath: &sourcePath,
	}))
	require.NoError(t, database.InsertMessages([]db.Message{{
		SessionID: previousID, Ordinal: 0, Role: "user", Content: "legacy archive",
	}}))
	...
	result := processResult{
		results: []parser.ParseResult{{Session: parser.ParsedSession{
			ID: currentID, Agent: agent, Machine: "local", Project: "project",
			StartedAt: started, EndedAt: started,
			File: parser.FileInfo{Path: sourcePath},
		}}},
		excludedSessionIDs: []string{previousID},
	...
	results <- syncJob{agent: agent, path: sourcePath, processResult: result}
	close(results)
	...
	legacy, err := database.GetSessionFull(t.Context(), previousID)
	require.NoError(t, err)
	require.NotNil(t, legacy,
		"a failed replacement must leave the legacy archive durable")
	messages, err := database.GetAllMessages(t.Context(), previousID)
	require.NoError(t, err)
	require.Len(t, messages, 1)
	assert.Equal(t, "legacy archive", messages[0].Content)
	current, err := database.GetSession(t.Context(), currentID)
	require.NoError(t, err)
	assert.Nil(t, current)
}

func TestIdentityMigrationConcurrentLegacyDeletionSuppressesReplacement(
	t *testing.T,
) {
	const (
		agent      parser.AgentType = "identity-concurrent-delete"
		previousID string           = "identity:legacy"
		currentID  string           = "identity:current"
	)
	tests := []struct {
		name      string
		remove    func(*testing.T, *db.DB, string)
		assertOld func(*testing.T, *db.DB, string)
	}{
		{
			name: "trash",
			remove: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				require.NoError(t, database.SoftDeleteSession(id))
			},
			assertOld: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				assert.True(t, database.IsSessionTrashed(id))
			},
		},
		{
			name: "permanent_exclusion",
			remove: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				require.NoError(t, database.DeleteSession(id))
			},
			assertOld: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				assert.True(t, database.IsSessionExcluded(id))
			},
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, database.InsertMessages([]db.Message{{
				SessionID: previousID, Ordinal: 0, Role: "user",
			...
			engine.writeBatchOverride = func(
				...
				written, messages, failed, filtered :=
					engine.writeBatch(batch, mode, force)
				require.Zero(t, failed)
				require.Zero(t, filtered)
				tc.remove(t, database, previousID)
				return written, messages, failed, filtered
			}
			started := time.Unix(1704067200, 0)
			result := processResult{
				results: []parser.ParseResult{{Session: parser.ParsedSession{
					ID: currentID, Agent: agent, Machine: "local",
					Project: "project", StartedAt: started, EndedAt: started,
					File: parser.FileInfo{Path: sourcePath},
				}}},
				excludedSessionIDs: []string{previousID},
				identityMigrations: []parser.SessionIdentityMigration{{
			...
			results := make(chan syncJob, 1)
			results <- syncJob{
				agent: agent, path: sourcePath, processResult: result,
			}
			close(results)
			...
			require.NoError(t, err)
			assert.Nil(t, current,
				"a deletion racing replacement durability must suppress the replacement")
			tc.assertOld(t, database, previousID)
		})
	}
}

func TestIdentityMigrationRetiresEveryTextPredecessorAfterReplacementWrite(
	t *testing.T,
) {
	const (
		agent     parser.AgentType = "identity-text-binary"
		currentID string           = "omnigent:7:1234567890abcdef1234567890abcdef"
	)
	previousIDs := []string{
		"omnigent:1234567890abcdef1234567890abcdef",
		"omnigent:conv_1234567890abcdef1234567890abcdef",
		"omnigent:7:conv_1234567890abcdef1234567890abcdef",
	}
	database := openTestDB(t)
	sourcePath := filepath.Join(t.TempDir(), "chat.db#7:binary")
	for _, id := range previousIDs {
		require.NoError(t, database.UpsertSession(db.Session{
			ID: id, Agent: string(agent), Machine: "local",
			Project: "project", FilePath: &sourcePath,
		}))
	}
	engine := NewEngine(database, EngineConfig{Machine: "local"})
	t.Cleanup(engine.Close)
	started := time.Unix(1704067200, 0)
	migrations := make([]parser.SessionIdentityMigration, 0, len(previousIDs))
	for _, id := range previousIDs {
		migrations = append(migrations, parser.SessionIdentityMigration{
			PreviousID: id, CurrentID: currentID,
		})
	}
	result := processResult{
		results: []parser.ParseResult{{Session: parser.ParsedSession{
		...
		excludedSessionIDs: previousIDs,
	...
	for _, id := range previousIDs {
		previous, err := database.GetSessionFull(t.Context(), id)
		require.NoError(t, err)
		assert.Nilf(t, previous, "predecessor %s must retire", id)
	}
}

func TestIdentityMigrationTextPredecessorDeletionSuppressesReplacement(
	t *testing.T,
) {
	const (
		agent     parser.AgentType = "identity-text-binary"
	...
	tests := []struct {
		...
		protected string
		remove    func(*testing.T, *db.DB, string)
		assert    func(*testing.T, *db.DB, string)
	}{
		{
			name:      "unqualified_text_trash",
			protected: previousIDs[1],
			remove: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				require.NoError(t, database.SoftDeleteSession(id))
			},
			assert: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				assert.True(t, database.IsSessionTrashed(id))
			},
		},
		{
			name:      "workspace_text_permanent_exclusion",
			protected: previousIDs[2],
			remove: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				require.NoError(t, database.DeleteSession(id))
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.remove(t, database, tc.protected)
			engine := NewEngine(database, EngineConfig{Machine: "local"})
			t.Cleanup(engine.Close)
			started := time.Unix(1704067200, 0)
			migrations := make(
				[]parser.SessionIdentityMigration, 0, len(previousIDs),
			)
			for _, id := range previousIDs {
				migrations = append(migrations, parser.SessionIdentityMigration{
					PreviousID: id, CurrentID: currentID,
			...
			require.NoError(t, err)
			assert.Nil(t, current)
			tc.assert(t, database, tc.protected)
		})
	}
}

func TestIdentityMigrationPreservesProtectedCurrentAndPredecessor(
	t *testing.T,
) {
	const (
		agent      parser.AgentType = "identity-both-protected"
		previousID string           = "identity:legacy"
		currentID  string           = "identity:current"
	)
	database := openTestDB(t)
	...
	for _, seed := range []struct {
		...
		{id: previousID, content: "legacy archive"},
		{id: currentID, content: "current archive"},
	} {
		require.NoError(t, database.UpsertSession(db.Session{
			ID: seed.id, Agent: string(agent), Machine: "local",
			Project: "project", FilePath: &sourcePath,
		}))
		require.NoError(t, database.InsertMessages([]db.Message{{
			SessionID: seed.id, Ordinal: 0, Role: "user", Content: seed.content,
		}}))
		require.NoError(t, database.SoftDeleteSession(seed.id))
	}
	engine := NewEngine(database, EngineConfig{Machine: "local"})
	t.Cleanup(engine.Close)
	started := time.Unix(1704067200, 0)
	...
	for _, want := range []struct {
		...
		assert.Truef(t, database.IsSessionTrashed(want.id),
			"%s must remain recoverable in trash", want.id)
		stored, err := database.GetSessionFull(t.Context(), want.id)
		require.NoError(t, err)
		require.NotNilf(t, stored, "%s row must remain in the archive", want.id)
		messages, err := database.GetAllMessages(t.Context(), want.id)
		require.NoError(t, err)
		require.Len(t, messages, 1)
...
func TestIdentityMigrationSuppressedCurrentCleanupRechecksProtection(
	t *testing.T,
) {
	const (
		agent      parser.AgentType = "identity-cleanup-race"
		previousID string           = "identity:legacy"
		currentID  string           = "identity:current"
		ordinaryID string           = "identity:ordinary-exclusion"
	)
	tests := []struct {
		name          string
		protect       func(*testing.T, *db.DB, string)
		assertCurrent func(*testing.T, *db.DB, string)
	}{
		{
			name: "trash",
			protect: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				require.NoError(t, database.SoftDeleteSession(id))
			},
			assertCurrent: func(t *testing.T, database *db.DB, id string) {
				t.Helper()
				assert.True(t, database.IsSessionTrashed(id))
				...
				require.NoError(t, err)
				require.Len(t, messages, 1)
				assert.Equal(t, "current archive", messages[0].Content)
			},
		},
		{
			name: "permanent_exclusion",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, seed := range []struct {
				...
				{id: previousID, content: "legacy archive"},
				{id: currentID, content: "current archive"},
				{id: ordinaryID, content: "ordinary parser exclusion"},
			} {
				require.NoError(t, database.UpsertSession(db.Session{
					ID: seed.id, Agent: string(agent), Machine: "local",
					Project: "project", FilePath: &sourcePath,
			...
			require.NoError(t, database.SoftDeleteSession(previousID))
			engine := NewEngine(database, EngineConfig{Machine: "local"})
			t.Cleanup(engine.Close)
			result := processResult{
				results: []parser.ParseResult{{Session: parser.ParsedSession{
					ID: currentID, Agent: agent, Machine: "local",
					Project: "project", File: parser.FileInfo{Path: sourcePath},
				}}},
				excludedSessionIDs: []string{previousID, ordinaryID},
				identityMigrations: []parser.SessionIdentityMigration{{
					PreviousID: previousID,
					CurrentID:  currentID,
				}},
			...
			sourceAllowed := engine.sourceAllowsParserExclusions(result)
			engine.applyProviderIdentityMigrationPolicies(&result)
			tc.protect(t, database, currentID)
			excluded, err := engine.deleteParserExcludedSessions(
				result, sourceAllowed,
			)

			require.NoError(t, err)
			assert.Equal(t, []string{ordinaryID}, excluded,
				"protected migration replacements must not be resync exclusions")
			ordinary, err := database.GetSessionFull(t.Context(), ordinaryID)
			require.NoError(t, err)
			assert.Nil(t, ordinary,
				"ordinary parser exclusions must retain hard-delete behavior")
			tc.assertCurrent(t, database, currentID)
			assert.True(t, database.IsSessionTrashed(previousID))
		})
	}
}

func TestIdentityMigrationBatchKeepsPrePolicyCwdAdmissionForCleanup(
	t *testing.T,
) {
	const (
		agent      parser.AgentType = "identity-cwd-batch"
		previousID string           = "identity:legacy"
		currentID  string           = "identity:current"
	)
	database := openTestDB(t)
	sourcePath := filepath.Join(t.TempDir(), "shared.db#current")
	for _, id := range []string{previousID, currentID} {
		require.NoError(t, database.UpsertSession(db.Session{
			ID: id, Agent: string(agent), Machine: "local",
			Project: "project", FilePath: &sourcePath,
		}))
	...
	result := processResult{
		results: []parser.ParseResult{{Session: parser.ParsedSession{
			ID: currentID, Agent: agent, Machine: "local", Project: "project",
			Cwd: "/allowed/repo", File: parser.FileInfo{Path: sourcePath},
		}}},
		excludedSessionIDs: []string{previousID},
		identityMigrations: []parser.SessionIdentityMigration{{
			PreviousID: previousID,
	...
	require.NoError(t, err)
	assert.Nil(t, current,
		"an in-scope suppressed current must still be cleaned up")
	assert.True(t, database.IsSessionTrashed(previousID))
	assert.Equal(t, []string{currentID}, stats.parserExcludedIDs)
}

func TestIdentityMigrationRestartRetiresPredecessorForUnchangedReplacement(
	t *testing.T,
) {
	const (
		previousID = "semantic:legacy"
		currentID  = "semantic:current"
	)
	database := openTestDB(t)
	root := t.TempDir()
	...
	fingerprint := parser.SourceFingerprint{
		Key: container, Size: 9, MTimeNS: 1234, Hash: "current-hash",
	}
	legacyPath := container + "#legacy"
	currentPath := container + "#current"
	size := fingerprint.Size
	mtime := fingerprint.MTimeNS
	hash := fingerprint.Hash
	require.NoError(t, database.UpsertSession(db.Session{
		ID: previousID, Agent: string(semanticTestAgent), Machine: "devbox",
		Project: "semantic-project", FilePath: &legacyPath,
	}))
	require.NoError(t, database.UpsertSession(db.Session{
		ID: currentID, Agent: string(semanticTestAgent), Machine: "devbox",
		Project: "semantic-project", FilePath: &currentPath,
		FileSize: &size, FileMtime: &mtime, FileHash: &hash,
	}))
	require.NoError(t, database.SetSessionDataVersion(
		currentID, db.CurrentDataVersion(),
	))
	currentResult := semanticTestResult(currentID, currentPath, fingerprint)
	currentResult.Result.Session.File.Hash = fingerprint.Hash
	provider := &semanticTestProvider{
		ProviderBase: parser.ProviderBase{
			...
			Caps: parser.Capabilities{
				...
				Sync: parser.ProviderSyncSemantics{
					...
					UnchangedResults:          parser.UnchangedResultMTimeAndHash,
				},
			},
		},
		...
		outcomes: map[string]parser.ParseOutcome{
			container: {
				Results: []parser.ParseResultOutcome{currentResult},
				SessionIdentityMigrations: []parser.SessionIdentityMigration{{
					PreviousID: previousID,
					CurrentID:  currentID,
				}},
	...
	err := engine.ReconcileProviderRoots(
		t.Context(), semanticTestAgent, []string{root},
	)

	require.NoError(t, err)
	assert.Equal(t, 1, provider.parseCalls,
		"the cold engine must parse before proving the current member unchanged")
	current, getErr := database.GetSession(t.Context(), currentID)
	require.NoError(t, getErr)
	require.NotNil(t, current)
	legacy, getErr := database.GetSessionFull(t.Context(), previousID)
	require.NoError(t, getErr)
	assert.Nil(t, legacy,
		"a durable unchanged replacement must retire an unbaselined predecessor")
	pending, generation := engine.fullDiscoveryRetry(semanticTestAgent)
	assert.False(t, pending)
	assert.Zero(t, generation,
		"restart recovery must not depend on an in-memory forced-retry latch")
}
