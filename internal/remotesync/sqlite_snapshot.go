...
func omnigentChatDBTargets(targets TargetSet) []string {
	seen := make(map[string]struct{})
	for _, path := range targets.Files[parser.AgentOmnigent] {
		chatDB, ok := omnigentChatDBForArchivePath(path)
		if ok {
			seen[chatDB] = struct{}{}
		}
	}
...
func omnigentSQLitePaths(chatDB string) []string {
	return []string{
		filepath.Clean(chatDB),
		filepath.Clean(chatDB + "-wal"),
		filepath.Clean(chatDB + "-shm"),
		filepath.Clean(chatDB + "-journal"),
	}
}

func omnigentChatDBForArchivePath(path string) (string, bool) {
	path = filepath.Clean(path)
	switch filepath.Base(path) {
	case parser.OmnigentDBName:
		return path, true
	case parser.OmnigentDBName + "-wal":
		return strings.TrimSuffix(path, "-wal"), true
	case parser.OmnigentDBName + "-shm":
		return strings.TrimSuffix(path, "-shm"), true
	case parser.OmnigentDBName + "-journal":
		return strings.TrimSuffix(path, "-journal"), true
	default:
		return "", false
	}
