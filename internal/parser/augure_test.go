...
func TestAugureSessionIDRelabel(t *testing.T) {
	tests := []struct {
		name string
		id   string
		want string
		...
		{"empty stays empty", "", ""},
		{"codex prefix", "codex:019fbcca", "augure:019fbcca"},
		{
			"already relabeled",
			"augure:019fbcca",
			"augure:019fbcca",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, augureSessionID(tt.id))
		})
	}
}

func TestRelabelCodexResultAsAugure(t *testing.T) {
	sess := &ParsedSession{
		ID:              "codex:child",
	...
	assert.Equal(t, "augure:origin", sess.SourceSessionID)
	assert.Equal(t, AgentAugure, sess.Agent)
	assert.Equal(
...
// TestRelabelCodexResultAsAugureIncremental covers the provider's
// incremental path, which has appended rows but no session to relabel.
func TestRelabelCodexResultAsAugureIncremental(t *testing.T) {
	msgs := []ParsedMessage{{
		ToolCalls: []ParsedToolCall{{
			SubagentSessionID: "codex:spawned",
		}},
...
func TestAugureRegistryEntry(t *testing.T) {
	def, ok := AgentByType(AgentAugure)
	require.True(t, ok)
	assert.Equal(t, "augure:", def.IDPrefix)
	assert.Equal(t, "AUGURE_SESSIONS_DIR", def.EnvVar)
	assert.Equal(t, "augure_sessions_dirs", def.ConfigKey)
	...
	assert.False(t, def.RemoteSyncExcluded,
		"Augure rollouts are plaintext and stay eligible for remote sync")

	// augure: must resolve through AgentByPrefix.
	byPrefix, ok := AgentByPrefix("augure:019fbcca")
	require.True(t, ok)
	assert.Equal(t, AgentAugure, byPrefix.Type)

	assert.Equal(t, []string{".augure/sessions"}, def.DefaultDirs)
	assert.Nil(t, def.ShallowWatchRootsFunc,
		"the shallow watch exists for Codex's session_index.jsonl only; Augure writes none")
}

// TestAugureProviderParsesDeidentifiedRollout runs the full Discover ->
...
// Augure rollouts are byte-compatible with the Codex format.
func TestAugureProviderParsesDeidentifiedRollout(t *testing.T) {
	root := t.TempDir()
	...
	provider, ok := NewProvider(AgentAugure, ProviderConfig{
		Roots:   []string{root},
		Machine: "devbox",
	})
	require.True(t, ok)
	...
	assert.Equal(t, AgentAugure, sources[0].Provider)
	...
	assert.Equal(t, AgentAugure, sess.Agent)
	assert.Equal(t, "augure:"+uuid, sess.ID)
...
// TestAugureProviderKeepsSeparateSourceKeys guards the discovery namespace:
// Codex, TraeX, and Augure share a UUID shape, so a shared source key would
// let one agent's session resolve to another's file.
func TestAugureProviderKeepsSeparateSourceKeys(t *testing.T) {
	const uuid = "3f2b1c9a-7d51-4e0a-9b2f-1c8a4d5e6f70"
	assert.NotEqual(
		t,
		CodexSourceKey(AgentCodex, uuid),
		CodexSourceKey(AgentAugure, uuid),
	)
	assert.NotEqual(
		t,
...
// TestAugureProviderIgnoresCodexSidecars covers the Codex-only out-of-band
// surfaces the shared provider must not expose to a fork: the
// session_index.jsonl watch, the index changed-path fan-out, and the
// s3://.../raw/codex archive layout. Augure writes none of them.
func TestAugureProviderIgnoresCodexSidecars(t *testing.T) {
	...
	// own this directory. It must not fan out to every Augure session.
	...
	assert.False(t, AgentSupportsS3Discovery(AgentAugure),
		"Augure has no S3 archive convention and must not import one as Codex")
	s3Provider, ok := S3ProviderFor(AgentAugure)
	assert.False(t, ok)
	...
	assert.Equal(t, RawCaptureCapabilities{}, caps.RawCapture,
		"Augure must not claim Codex raw capture")
}
