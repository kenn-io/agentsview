...
func TestProviderCapabilitiesActivityHintsMatchConsumers(t *testing.T) {
	...
	for _, factory := range ProviderFactories() {
		...
		// TraeX and Augure write the same history.jsonl at the same position
		// relative to their sessions roots, so they inherit the Codex hint
		// reader.
		if agent == AgentCodex || agent == AgentTraeX || agent == AgentAugure {
			assert.Equal(t, CapabilitySupported, got)
			provider := factory.NewProvider(ProviderConfig{
				Roots: []string{t.TempDir()},
			})
