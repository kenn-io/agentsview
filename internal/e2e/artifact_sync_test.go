...
func TestArtifactSyncTwoInstanceFolderAndHTTP(t *testing.T) {
	...
	_, err := artifact.ExportToStore(ctx, a.DB, a.Repository.Store(), artifact.ExportOptions{
		Origin: a.Origin,
		Full:   true,
	})
	...
	imported, err := artifact.ImportDetailedFromStore(ctx, b.DB, b.Repository.Store(), b.Origin)
	require.NoError(t, err)
...
func openE2ENode(t *testing.T, dataDir, origin string) *e2eNode {
	...
	srv := server.New(cfg, database, engine,
		...
		server.WithArtifactStore(repository.Store()),
	)
	return &e2eNode{
		DataDir:    dataDir,
		DBPath:     dbPath,
...
func folderSync(t *testing.T, n *e2eNode, share string) artifact.SyncResult {
	t.Helper()
	res, err := artifact.SyncWithStore(context.Background(), n.DB, n.Repository.Store(), artifact.SyncOptions{
		DataDir: n.DataDir,
		Target:  share,
		Origin:  n.Origin,
	})
...
func renameWithMetadata(t *testing.T, n *e2eNode, sessionID, displayName string) {
	...
	appendRenameArtifact(t, n.DB, n.Repository.Store(), n.Origin, sessionID, displayName)
}
...
func writeForeignRename(t *testing.T, n *e2eNode, origin, sessionID, displayName string) {
	...
	appendRenameArtifact(t, scratch, n.Repository.Store(), origin, sessionID, displayName)
}
...
func postOriginArtifacts(t *testing.T, from, to *e2eNode, origin string) {
	...
	for _, kind := range []artifact.Kind{
		...
		page, err := from.Repository.Store().List(t.Context(), origin, kind, "", 100)
		require.NoError(t, err)
		require.Empty(t, page.Next)
		for _, entry := range page.Items {
			_, reader, err := from.Repository.Store().Open(t.Context(), entry.Ref)
			require.NoError(t, err)
