...
func (i *storeOriginIterator) Next(ctx context.Context) (string, bool, error) {
	for {
		...
		origins, next, err := i.store.ListOrigins(ctx, i.cursor, transportPageSize)
		if err != nil {
			return "", false, err
		}
		...
		if next == "" {
			i.done = true
		} else if i.guard.Observe(next) {
			return "", false, errors.New("artifact origin cursor cycle")
		}
	}
}
...
func (i *storeWireIterator) Next(ctx context.Context) (WireRef, Entry, bool, error) {
	for {
		...
		page, err := i.store.List(ctx, i.origin, transportKinds[i.kindIndex], i.cursor, transportPageSize)
		if err != nil {
		...
		if i.cursor == "" {
			...
			return WireRef{}, Entry{}, false, errors.New("artifact list cursor cycle")
		}
	}
}
