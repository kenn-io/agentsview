...
type artifactStoreImportScopeKey struct {
	typeName string
	pointer  uintptr
}

type artifactStoreImportScope struct {
	gate  chan struct{}
	refs  int
	owner any
}

var artifactStoreImportScopes = struct {
	sync.Mutex
	items map[artifactStoreImportScopeKey]*artifactStoreImportScope
}{items: make(map[artifactStoreImportScopeKey]*artifactStoreImportScope)}
...
func SyncWithStore(
	...
	tr, err := syncTransport(store, opts)
	if err != nil {
		return SyncResult{}, err
	}
	return syncStoreWithTransport(ctx, database, store, opts, tr)
}

func syncTransport(store ArtifactStore, opts SyncOptions) (Transport, error) {
	...
	provider, ok := store.(ArtifactFolderTransportProvider)
	if !ok || provider == nil || isTypedNil(provider) {
		return nil, fmt.Errorf(
			"%w: folder exchange requires retained repository identity",
			ErrArtifactUnsupported,
		)
	}
	transport, err := provider.NewFolderTransport(opts.Target)
	if err != nil {
		return nil, err
	}
...
func syncFolderWithOwnedStore(
	...
	localStore := repository.Store()
	transport, err := syncTransport(localStore, opts)
	if err != nil {
		return SyncResult{}, err
	}
	return syncStoreWithTransport(ctx, database, localStore, opts, transport)
}
...
func syncWithTransport(
	...
	defer func() { retErr = errors.Join(retErr, repository.Close()) }()
	return syncStoreWithTransport(ctx, database, repository.Store(), opts, tr)
}

func syncStoreWithTransport(
	ctx context.Context,
	database *db.DB,
	...
	defer func() {
		if retErr == nil {
			NotifyArtifactBatch(notificationCtx, localStore)
		}
	}()
	...
	if _, _, err := RecoverRepositoryResetRepublish(ctx, database, localStore, origin); err != nil {
		return SyncResult{}, fmt.Errorf("recovering interrupted artifact repository reset: %w", err)
	}
	...
	if opts.BaselineMetadata {
		...
		if err := coordinator.ScheduleArtifactImport(); err != nil {
			return SyncResult{}, err
		}
	...
	exported, err := PublishRepositoryArtifacts(ctx, database, localStore, ExportOptions{
		Origin: origin,
		Full:   true,
	})
	if err != nil {
...
func ExportToStore(
	...
	if !opts.skipMaintenanceNotify {
		defer func() {
			if retErr == nil {
				NotifyArtifactBatch(ctx, store)
			}
		}()
	}
	if opts.Full {
...
func exportFullToStore(
	...
	drain := func() error {
		for {
			...
			page, err := ExportToStore(ctx, database, store, ExportOptions{
				Origin: origin, SessionIDs: ids, skipMaintenanceNotify: true,
			})
			if err != nil {
				return err
			}
	...
	for {
		final, err := ExportToStore(ctx, database, store, ExportOptions{
			Origin: origin, skipMaintenanceNotify: true,
		})
		if err != nil {
			return result, err
		}
...
func latestValidCheckpointHead(
	...
	for {
		page, err := store.List(ctx, origin, KindCheckpoints, cursor, checkpointFloorPageSize)
		if err != nil {
		...
		if cursors.Observe(cursor) {
			return db.ArtifactCheckpointHead{}, false, errors.New("artifact checkpoint listing returned a repeated cursor")
		}
	}
...
func reserveCheckpointSequenceFromStore(
	...
	if observer, ok := store.(checkpointFloorStore); ok {
		...
		for {
			page, err := store.List(
				ctx, origin, KindCheckpoints, cursor, checkpointFloorPageSize,
			)
			if err != nil {
				return 0, fmt.Errorf("listing checkpoint floor for %s: %w", origin, err)
			}
			...
			if cursors.Observe(cursor) {
				return 0, errors.New("artifact checkpoint listing returned a repeated cursor")
			}
		}
	}
...
func ImportDetailedFromStore(
	ctx context.Context,
	database *db.DB,
	store ArtifactStore,
	localOrigin string,
) (ImportResult, error) {
	return importDetailedFromStore(ctx, database, store, localOrigin, HLCClockOptions{})
}
...
func importDetailedFromStore(
	...
	store ArtifactStore,
	localOrigin string,
	clockOptions HLCClockOptions,
) (ImportResult, error) {
	return importDetailedFromStoreWithClock(
		ctx, database, store, localOrigin, NewHLCClock(database, clockOptions),
	)
}

func importDetailedFromStoreWithClock(
	ctx context.Context,
	database *db.DB,
	store ArtifactStore,
	localOrigin string,
	...
	if database == nil {
		return ImportResult{}, errors.New("artifact import database is required")
	}
	if store == nil {
		return ImportResult{}, errors.New("artifact import store is required")
	}
	if err := validateOriginID(localOrigin); err != nil {
		return ImportResult{}, err
	}
	releaseImportScope, err := acquireArtifactStoreImportScope(ctx, database, store)
	if err != nil {
		return ImportResult{}, err
	}
	defer releaseImportScope()
	var result ImportResult
	err = visitStoreOrigins(ctx, store, localOrigin, func(origin string) error {
		originResult, err := importOriginFromStore(ctx, database, store, origin)
		result.Sessions += originResult.Sessions
		result.Messages += originResult.Messages
		result.Deferred += originResult.Deferred
		return err
	})
	if err != nil {
		return result, err
	...
	err = visitStoreOrigins(ctx, store, localOrigin, func(origin string) error {
		metadata, err := replayMetadataFromStore(
			ctx, database, clock, store, origin, localOrigin,
		)
...
func acquireArtifactStoreImportScope(
	ctx context.Context, database *db.DB, store ArtifactStore,
) (func(), error) {
	key, owner := artifactStoreImportScopeKeyFor(database, store)
	artifactStoreImportScopes.Lock()
	scope := artifactStoreImportScopes.items[key]
	if scope == nil {
		scope = &artifactStoreImportScope{gate: make(chan struct{}, 1), owner: owner}
		artifactStoreImportScopes.items[key] = scope
	}
	scope.refs++
	artifactStoreImportScopes.Unlock()

	select {
	case scope.gate <- struct{}{}:
	case <-ctx.Done():
		releaseArtifactStoreImportScopeRef(key, scope)
		return nil, ctx.Err()
	}
	return func() {
		<-scope.gate
...
func artifactStoreImportScopeKeyFor(
	database *db.DB, store ArtifactStore,
) (artifactStoreImportScopeKey, any) {
	if provider, ok := store.(ArtifactImportScopeProvider); ok {
		if scope := provider.ArtifactImportScope(); scope != nil {
			return artifactStoreImportScopeKey{
				typeName: "artifact-import-scope",
				pointer:  reflect.ValueOf(scope).Pointer(),
			}, scope
		}
	}
	value := reflect.ValueOf(store)
	key := artifactStoreImportScopeKey{typeName: value.Type().String()}
	switch value.Kind() {
	...
	return artifactStoreImportScopeKey{
		typeName: "artifact-import-fallback-database",
		pointer:  reflect.ValueOf(database).Pointer(),
	}, database
}

func releaseArtifactStoreImportScopeRef(
	key artifactStoreImportScopeKey, scope *artifactStoreImportScope,
) {
	artifactStoreImportScopes.Lock()
	scope.refs--
	if scope.refs == 0 && artifactStoreImportScopes.items[key] == scope {
		delete(artifactStoreImportScopes.items, key)
	}
	artifactStoreImportScopes.Unlock()
}

func visitStoreOrigins(
	ctx context.Context,
	store ArtifactStore,
	localOrigin string,
	...
	var cursors boundedCursorCycleGuard
	for {
		origins, next, err := store.ListOrigins(ctx, cursor, artifactImportPageSize)
		if err != nil {
			return fmt.Errorf("listing artifact import origins: %w", err)
		}
		cursor = next
		...
		if cursors.Observe(cursor) {
			return errors.New("artifact origin listing returned a repeated cursor")
		}
	}
}

func importOriginFromStore(
	ctx context.Context,
	database *db.DB,
	store ArtifactStore,
	origin string,
) (_ ImportResult, retErr error) {
	spool, err := spoolStoreEntries(ctx, store, origin, KindCheckpoints)
	if err != nil {
		return ImportResult{}, fmt.Errorf("listing %s artifacts for %s: %w",
			KindCheckpoints, origin, err)
	}
	defer func() {
	...
	deferred := 0
	for {
		entry, ok, err := spool.NextReverse()
		if err != nil {
			return ImportResult{Deferred: deferred}, err
		}
		if !ok {
			break
		}
		...
		data, err := readVerifiedStoreArtifact(
			ctx, database, store, entry, checkpointDecodedLimit,
		)
		...
		if err != nil {
			if errors.Is(err, ErrArtifactInvalid) {
				if qerr := store.Quarantine(ctx, entry.Ref, err.Error()); qerr != nil {
					return ImportResult{}, errors.Join(err, qerr)
				}
				continue
			}
			return ImportResult{}, fmt.Errorf("reading checkpoint %s: %w", entry.Ref.Name, err)
		}
		var cp checkpoint
		if err := json.Unmarshal(data, &cp); err != nil {
			if qerr := store.Quarantine(ctx, entry.Ref, "checkpoint JSON is invalid"); qerr != nil {
				return ImportResult{}, errors.Join(err, qerr)
			}
			continue
		}
		...
		if canonicalErr != nil || !bytes.Equal(canonical, data) {
			if qerr := store.Quarantine(ctx, entry.Ref, "checkpoint JSON is not canonical"); qerr != nil {
				return ImportResult{}, errors.Join(canonicalErr, qerr)
			}
			continue
		}
		...
		outcome, err := inspectCheckpointClosureFromStore(ctx, database, store, origin, cp)
		if err != nil {
			return ImportResult{Deferred: deferred}, err
		}
	...
	if selected == nil {
		return ImportResult{Deferred: deferred}, nil
	}
	result, err := importCheckpointFromStore(ctx, database, store, origin, *selected)
	result.Deferred += deferred
	return result, err
}

const maxSpooledEntryBytes = 64 << 10

type storeEntrySpool struct {
	file *os.File
	pos  int64
}

var storeEntrySpoolCleanup = func(spool *storeEntrySpool) error {
	return spool.close()
}

func spoolStoreEntries(
	ctx context.Context, store ArtifactStore, origin string, kind Kind,
) (_ *storeEntrySpool, retErr error) {
	file, err := os.CreateTemp("", "agentsview-artifact-entries-*")
	if err != nil {
		return nil, err
	}
	spool := &storeEntrySpool{file: file}
	defer func() {
		if retErr != nil {
			retErr = errors.Join(retErr, spool.Close())
		}
	...
	var cursors boundedCursorCycleGuard
	for {
		page, err := store.List(ctx, origin, kind, cursor, artifactImportPageSize)
		if err != nil {
			return nil, err
		}
		cursor = page.Next
		for _, entry := range page.Items {
			...
			if len(data) > maxSpooledEntryBytes {
				return nil, fmt.Errorf("artifact entry metadata exceeds %d bytes", maxSpooledEntryBytes)
			}
		...
		if cursors.Observe(cursor) {
			return nil, errors.New("artifact listing returned a repeated cursor")
		}
	}
}
...
func (s *storeEntrySpool) NextReverse() (Entry, bool, error) {
	if s == nil || s.file == nil || s.pos == 0 {
		return Entry{}, false, nil
	}
	if s.pos < 8 {
		return Entry{}, false, errors.New("artifact entry spool is truncated")
	}
	var size [8]byte
	if _, err := s.file.ReadAt(size[:], s.pos-8); err != nil {
		return Entry{}, false, err
	}
	length := int64(binary.BigEndian.Uint64(size[:]))
	if length < 0 || length > maxSpooledEntryBytes || length > s.pos-8 {
		return Entry{}, false, errors.New("artifact entry spool record is invalid")
	}
	start := s.pos - 8 - length
...
func (s *storeEntrySpool) Close() error {
	return storeEntrySpoolCleanup(s)
}
...
func releaseArtifactCursor(store ArtifactStore, cursor *Cursor) error {
	if cursor == nil || *cursor == "" {
		return nil
	}
	var err error
	if releaser, ok := store.(ArtifactCursorReleaser); ok {
		err = releaser.ReleaseCursor(*cursor)
	}
	*cursor = ""
	return err
}
...
func (s *coordinatedTransportStore) Create(
	ctx context.Context,
	ref Ref,
	identity Identity,
	mediaType string,
	...
	if err := s.coordinator.ScheduleArtifactImport(); err != nil {
		return result, err
	}
	return result, nil
}

func (s *coordinatedTransportStore) NotifyArtifactBatch(ctx context.Context) {
	NotifyArtifactBatch(ctx, s.ArtifactStore)
}
...
func (s *coordinatedTransportStore) RepairTransportArtifact(
	...
	return RepairArtifactFromTrustedPeer(ctx, s.database, s.ArtifactStore,
		db.ArtifactRepair{
			Origin: entry.Ref.Origin,
			Kind:   string(entry.Ref.Kind),
			Name:   entry.Ref.Name,
...
// ScheduleArtifactImport marks the current transfer batch for one import.
func (c *StoreImportCoordinator) ScheduleArtifactImport() error {
	if c == nil {
...
func (c *StoreImportCoordinator) Finalize(ctx context.Context) (ImportResult, error) {
	...
	result, err := importDetailedFromStore(
		ctx, c.database, c.store, c.localOrigin, HLCClockOptions{Now: c.now},
	)
	if err == nil {
		c.mu.Lock()
...
func RepairArtifactFromTrustedPeer(
	...
	repairer, ok := store.(artifactContentRepairer)
	if !ok {
	...
	if err := retry.ScheduleArtifactImport(); err != nil {
