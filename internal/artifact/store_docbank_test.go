...
func TestDocbankStoreDistinctReferencesCountOnePhysicalWrite(t *testing.T) {
	...
	assert.Never(t, func() bool { return packer.calls.Load() != 0 },
		...
		"one deduplicated physical object must remain below the two-object threshold")
}
...
func TestDocbankStoreReleaseCursorPropagatesWalkerCloseError(t *testing.T) {
	closeErr := errors.New("close retained walker")
	handle := strings.Repeat("a", 32)
	store := &docbankStore{
		traversals: map[string]*docbankTraversal{
			handle: {walker: &docbankWalkerStub{closeErr: closeErr}, generation: 1},
		},
		traversalOrder: []string{handle},
	}

	err := store.ReleaseCursor(encodeDocbankCursor(handle, 1))
	assert.ErrorIs(t, err, closeErr)
	assert.Empty(t, store.traversals)
}
...
func newProtocolTestStore(root string) (*docbankStore, error) {
	...
	return newDocbankStore(vault), nil
}
