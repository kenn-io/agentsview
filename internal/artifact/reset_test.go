...
func (t *resetRecoveryObservingTransport) Exchange(ctx context.Context, store ArtifactStore) error {
	...
	page, err := store.List(ctx, t.origin, KindMeta, "", 10)
	if err != nil {
		return err
	}
	if len(page.Items) != 1 {
...
func TestArtifactResetStoppedDaemonMovesAsideAndRepublishesFromSQLiteFloor(t *testing.T) {
	...
	repository, err := openRepository(t.Context(), dataDir, modernc.Driver{})
	require.NoError(t, err)
	first, err := ExportToStore(t.Context(), database, repository.Store(), ExportOptions{
	...
	_, err = repository.Store().Create(t.Context(), foreignRef, identityForBytes(t, foreignBody),
	...
	assert.Equal(t, []string{origin}, listAllContractOrigins(t, fresh.Store(), 10))
	assert.NotEmpty(t, listAllContractEntries(t, fresh.Store(), origin, KindMeta, 10),
		"reset must publish local curation that existed only in SQLite")
	_, err = fresh.Store().Stat(t.Context(), foreignRef)
	require.ErrorIs(t, err, ErrArtifactNotFound)
	replayDB := testDB(t)
	replay, err := ImportDetailedFromStore(t.Context(), replayDB, fresh.Store(), "receiver-a1b2c3")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	diagnosticStore := newDocbankStore(diagnostic)
...
func TestArtifactResetRematerializesCoveredLocalCurationBeforeFreshCheckpoint(t *testing.T) {
	...
	_, err = ExportToStore(ctx, database, bootstrap.Store(), ExportOptions{Origin: origin, Full: true})
	require.NoError(t, err)
	...
	replayDB := testDB(t)
	_, err = ImportDetailedFromStore(ctx, replayDB, bootstrap.Store(), "receiver-a1b2c3")
	require.NoError(t, err)
	...
	replayedResult, err := ImportDetailedFromStore(
		ctx, replayDB, fresh.Store(), "receiver-a1b2c3",
	)
	require.NoError(t, err)
...
func TestArtifactResetRematerializesLocalNegativeWinnersWithoutReattributingForeign(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, bootstrap.Close()) })
	_, err = ExportToStore(ctx, database, bootstrap.Store(), ExportOptions{
		Origin: localOrigin,
		Full:   true,
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = ImportDetailedFromStore(ctx, database, peerRepository.Store(), localOrigin)
	require.NoError(t, err)
	...
	metadataEntries := listAllContractEntries(t, fresh.Store(), localOrigin, KindMeta, 10)
	...
	for _, entry := range metadataEntries {
		data := readContractArtifact(t, fresh.Store(), entry.Ref)
	...
	result, err := ImportDetailedFromStore(ctx, replayDB, fresh.Store(), "receiver-a1b2c3")
	require.NoError(t, err)
...
func TestArtifactResetRepublishCrashAcrossWinnerPageIsIdempotent(t *testing.T) {
	...
	entries := listAllContractEntries(t, reopened.Store(), localOrigin, KindMeta, 64)
	...
	assert.Empty(t, listAllContractEntries(t, reopened.Store(), peerOrigin, KindMeta, 64),
...
func TestArtifactResetRepublishCrashAfterBaselineCreateDoesNotGrowEvents(t *testing.T) {
	...
	firstCtx, cancel := context.WithCancel(t.Context())
	interruptingStore := &cancelAfterMetadataCreateStore{
		ArtifactStore: repository.Store(), cancel: cancel, remaining: 1,
	}
	firstRecorder := NewMetadataRecorder(database, MetadataRecorderOptions{
		Origin: localOrigin,
		Store:  interruptingStore,
	...
	entries := listAllContractEntries(t, reopened.Store(), localOrigin, KindMeta, 10)
...
func TestArtifactResetRepublishCrashAcrossUncoveredBaselinePageIsIdempotent(t *testing.T) {
	...
	firstCtx, cancel := context.WithCancel(t.Context())
	interruptingStore := &cancelAfterMetadataCreateStore{
		ArtifactStore: repository.Store(), cancel: cancel, remaining: 128,
	}
	firstRecorder := NewMetadataRecorder(database, MetadataRecorderOptions{
		Origin: origin,
		Store:  interruptingStore,
	...
	_, recovered, err := RecoverRepositoryResetRepublish(
		t.Context(), database, reopened.Store(), origin,
	)
	require.NoError(t, err)
	assert.True(t, recovered)
	assert.Len(t, listAllContractEntries(t, reopened.Store(), origin, KindMeta, 64), baselineRows,
...
func TestRecoverRepositoryResetRepublishClearsMarkerAfterCheckpoint(t *testing.T) {
	...
	result, recovered, err := RecoverRepositoryResetRepublish(
		t.Context(), database, repository.Store(), origin,
	)
	require.NoError(t, err)
	assert.True(t, recovered)
	assert.Positive(t, result.CheckpointSequence)
	assert.Len(t, listAllContractEntries(t, repository.Store(), origin, KindMeta, 10), 1)
...
func TestRecoverRepositoryResetRepublishDoesNotConsumeMarkerForCompatibilityStore(t *testing.T) {
	dataDir := t.TempDir()
	database := openArtifactResetDB(t, dataDir)
	origin := "desktop-d4e5f6"
	want, err := PrepareRepositoryResetRepublish(t.Context(), database, dataDir, origin)
	require.NoError(t, err)
	compatibility, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, compatibility.Close()) })

	_, recovered, err := RecoverRepositoryResetRepublish(
		t.Context(), database, compatibility, origin,
	)
	require.NoError(t, err)
	assert.False(t, recovered)
...
func TestSyncStoreRecoversRepositoryResetBeforeFirstExchange(t *testing.T) {
	dataDir := t.TempDir()
	database := openArtifactResetDB(t, dataDir)
	origin := "desktop-d4e5f6"
	recordResetMetadataWinner(
	...
	_, err = syncStoreWithTransport(
		t.Context(), database, repository.Store(), SyncOptions{Origin: origin}, transport,
	)
	require.NoError(t, err)
...
func TestArtifactResetDaemonOwnerTransfersRepositoryReservation(t *testing.T) {
	...
	assert.DirExists(t, result.DiagnosticRoot)
	_, _, err = oldStore.ListOrigins(t.Context(), "", 10)
	require.Error(t, err)
	contender, err := OpenRepository(t.Context(), dataDir)
	assert.Nil(t, contender)
	require.ErrorContains(t, err, "ownership lock")
...
func TestArtifactResetFailureBoundariesPreserveRecoveryPaths(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name: "fresh root open",
			hooks: func(_ *testing.T, injected error) repositoryResetHooks {
				return repositoryResetHooks{
					now:      fixedArtifactResetTime,
					openRoot: func(string) (*os.Root, error) { return nil, injected },
				}
			},
			wantMoved: true, wantSource: true,
		},
