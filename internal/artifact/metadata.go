...
// ImportFromStore imports foreign artifacts through the recorder's canonical
// store while sharing the recorder's persisted HLC time source.
func (r *MetadataRecorder) ImportFromStore(
	...
	if store == nil {
		return ImportResult{}, errors.New("metadata recorder artifact store is required")
	}
	origin, err := r.resolveOrigin()
	if err != nil {
		return ImportResult{}, err
	}
	return importDetailedFromStoreWithClock(ctx, r.database, store, origin, r.clock)
}
