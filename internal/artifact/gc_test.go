...
func TestGarbageCollectExpiresQuarantineThroughLogicalCapability(t *testing.T) {
	...
	items, next, err := store.ListQuarantined(t.Context(), "", 1)
	require.NoError(t, err)
	...
	items, _, err = store.ListQuarantined(t.Context(), "", 1)
	require.NoError(t, err)
...
func TestGarbageCollectCheckpointReachabilityHeapIsBoundedByWorkingSet(t *testing.T) {
	small := measureCheckpointReachabilityHeap(t, 10)
	large := measureCheckpointReachabilityHeap(t, 10_000)

	assert.LessOrEqual(t, large, small+int64(1<<20),
		"10,000 checkpoint sessions must not leave a full Go map live during dependency reads")
}

func TestGCReachabilityIndexIsPrivateAndRemovedOnClose(t *testing.T) {
	closure, err := newGCOriginClosure(t.Context())
	require.NoError(t, err)
	dir := closure.dir
	path := closure.path
	...
	require.NoError(t, closure.Close())
	_, err = os.Stat(dir)
...
func (s *failingRetentionStore) ListOrigins(context.Context, Cursor, int) ([]string, Cursor, error) {
	return nil, "", s.err
}
...
func (s *checkpointHeapStore) ListOrigins(
	context.Context, Cursor, int,
) ([]string, Cursor, error) {
	return []string{retentionOrigin}, "", nil
}

func (s *checkpointHeapStore) List(
	_ context.Context, _ string, kind Kind, _ Cursor, _ int,
) (Page, error) {
	if kind == KindCheckpoints {
		return Page{Items: []Entry{s.entry}}, nil
	}
	return Page{}, nil
}
