...
func (t *s3Transport) exchangeOrigin(
	...
	remoteIt := &s3AllWireIterator{transport: t, origin: origin}
	defer func() { retErr = errors.Join(retErr, localIt.Close()) }()
...
type s3AllWireIterator struct {
	transport *s3Transport
	origin    string
	kindIndex int
	iterator  *s3WireIterator
}

func (i *s3AllWireIterator) Next(ctx context.Context) (WireRef, bool, error) {
	for i.kindIndex < len(transportKinds) {
		if i.iterator == nil {
			i.iterator = &s3WireIterator{
				transport: i.transport, origin: i.origin, kind: transportKinds[i.kindIndex],
			}
		}
		wire, ok, err := i.iterator.Next(ctx)
		if err != nil || ok {
			return wire, ok, err
		}
		i.iterator = nil
