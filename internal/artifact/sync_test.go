...
type artifactEntryOverrideStore struct {
	ArtifactStore
	ref   Ref
	entry Entry
}
...
func (s *originTraversalGateStore) ListOrigins(
	_ context.Context, cursor Cursor, _ int,
) ([]string, Cursor, error) {
	if cursor == "" {
		...
		return origins, "origin-page-2", nil
	}
	if cursor != "origin-page-2" {
		return nil, "", errors.New("unexpected origin cursor")
	}
	if !s.processedFirstPage {
		return nil, "", errors.New("second origin page requested before first page was processed")
	}
	return []string{"peer-ffffff"}, "", nil
}

func (s *originTraversalGateStore) List(
	_ context.Context, _ string, _ Kind, _ Cursor, _ int,
) (Page, error) {
	s.processedFirstPage = true
	return Page{}, nil
}

type entryTraversalGateStore struct {
	ArtifactStore
	kind  Kind
	opens int
}

func (s *entryTraversalGateStore) List(
	ctx context.Context, origin string, kind Kind, cursor Cursor, limit int,
) (Page, error) {
	if kind == s.kind && cursor != "" && s.opens == 0 {
		return Page{}, errors.New("next artifact page requested before current page was processed")
	}
	return s.ArtifactStore.List(ctx, origin, kind, cursor, limit)
}

func (s *entryTraversalGateStore) Open(
	ctx context.Context, ref Ref,
) (Entry, VerifiedReader, error) {
	if ref.Kind == s.kind {
		s.opens++
...
func (s *checkpointHeaderFirstStore) List(
	ctx context.Context, origin string, kind Kind, cursor Cursor, limit int,
) (Page, error) {
	page, err := s.ArtifactStore.List(ctx, origin, kind, cursor, limit)
	if err == nil && kind == KindCheckpoints && page.Next == "" {
		s.listedAll = true
	}
	return page, err
}
...
type cursorCycleStore struct {
	ArtifactStore
	cycleOrigins bool
	calls        int
}

type importConcurrencyStore struct {
	ArtifactStore
...
type importScopeAliasStore struct {
	ArtifactStore
	probe *importConcurrencyStore
}

func (s *importScopeAliasStore) ArtifactImportScope() *ArtifactImportScope {
	provider, ok := s.ArtifactStore.(ArtifactImportScopeProvider)
	if !ok {
		return nil
	}
...
func (s *importScopeAliasStore) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	return s.probe.ListOrigins(ctx, cursor, limit)
}

type valueImportStore struct {
	ArtifactStore
}

func (s *importConcurrencyStore) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	...
	return s.ArtifactStore.ListOrigins(ctx, cursor, limit)
}
...
func (s *failingImportScheduler) ScheduleArtifactImport() error {
	s.calls++
	return s.err
}
...
func (s *releaseFailureStore) ListOrigins(
	context.Context, Cursor, int,
) ([]string, Cursor, error) {
	return []string{"peer-a1b2c3"}, "retained-cursor", nil
}

func (s *releaseFailureStore) ReleaseCursor(Cursor) error { return s.releaseErr }

func (s *transientImportStore) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	if s.calls.Add(1) == 1 {
...
func (s *cursorCycleStore) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	if !s.cycleOrigins {
		return []string{"peer-a1b2c3"}, "", nil
	}
	s.calls++
	if s.calls > 4 {
		return nil, "", errors.New("cursor cycle was not detected")
	}
	switch cursor {
	case "":
		return nil, "cursor-a", nil
	case "cursor-a":
		return nil, "cursor-b", nil
	default:
		return nil, "cursor-a", nil
	}
}

func (s *cursorCycleStore) List(
	ctx context.Context, origin string, kind Kind, cursor Cursor, limit int,
) (Page, error) {
	if s.cycleOrigins || kind != KindCheckpoints {
		return s.ArtifactStore.List(ctx, origin, kind, cursor, limit)
	}
	s.calls++
	if s.calls > 4 {
		return Page{}, errors.New("cursor cycle was not detected")
	}
	switch cursor {
	case "":
		return Page{Next: "cursor-a"}, nil
	case "cursor-a":
		return Page{Next: "cursor-b"}, nil
	default:
		return Page{Next: "cursor-a"}, nil
	}
}
...
func (s *artifactEntryOverrideStore) Stat(ctx context.Context, ref Ref) (Entry, error) {
	if ref == s.ref {
		return s.entry, nil
	}
...
func (s *checkpointReadCountingStore) List(
	ctx context.Context, origin string, kind Kind, cursor Cursor, limit int,
) (Page, error) {
...
func TestExportToStoreFullRepairsMissingDependencyWithoutNewCheckpoint(t *testing.T) {
	...
	segments, err := filesystem.List(t.Context(), contractOrigin, KindSegments, "", 10)
	require.NoError(t, err)
...
func TestExportToStoreBootstrapsMissingDatabaseHeadFromLatestCheckpoint(t *testing.T) {
	...
	page, err := filesystem.List(t.Context(), contractOrigin, KindCheckpoints, "", 10)
	require.NoError(t, err)
...
func TestExportToStoreFailureKeepsClaimAndCheckpointLast(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			page, err := store.List(t.Context(), contractOrigin, KindCheckpoints, "", 10)
			require.NoError(t, err)
...
func TestExportToStoreStaleGenerationAfterCheckpointDoesNotAdvanceHead(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, pending, 1)
	firstPage, err := filesystem.List(t.Context(), contractOrigin, KindCheckpoints, "", 10)
	require.NoError(t, err)
...
func TestImportDetailedFromStoreDefersCheckpointWithAbsentDependency(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	...
	segments, err := store.List(t.Context(), origin, KindSegments, "", 10)
	require.NoError(t, err)
	require.Len(t, segments.Items, 1)
	require.NoError(t, store.Trash(t.Context(), segments.Items[0].Ref))

	target := testDB(t)
	result, err := ImportDetailedFromStore(
...
func (s *corruptUntilRepairedStore) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	s.originLists++
	return s.ArtifactStore.ListOrigins(ctx, cursor, limit)
...
func TestImportDetailedFromStoreRepairsPhysicalCorruptionFromTrustedPeer(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	...
	require.NoError(t, err)
	segments, err := canonical.List(t.Context(), origin, KindSegments, "", 10)
	require.NoError(t, err)
	require.Len(t, segments.Items, 1)
	...
	target := testDB(t)
	_, err = ImportDetailedFromStore(t.Context(), target, store, "local-d4e5f6")
	...
	require.NoError(t, err)
	assert.Empty(t, pending)
	coordinator.ScheduleArtifactImport()

	result, err := coordinator.Finalize(t.Context())
	require.NoError(t, err)
	assert.Equal(t, 1, result.Sessions)
	assert.Equal(t, 2, result.Messages)
	assert.Equal(t, 3, store.originLists,
		"the failed content pass plus one coalesced content/metadata retry are bounded traversals")

	result, err = coordinator.Finalize(t.Context())
	require.NoError(t, err)
	assert.False(t, result.Changed())
	assert.Equal(t, 3, store.originLists, "a drained coordinator must not run another import")
}
...
func TestStoreImportCoordinatorSerializesFinalizersAndRetainsMidRunWork(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
	store := &importConcurrencyStore{
		ArtifactStore: filesystem,
		entered:       make(chan struct{}),
		release:       make(chan struct{}),
	}
	coordinator := NewStoreImportCoordinator(testDB(t), store, "local-d4e5f6")
	coordinator.ScheduleArtifactImport()
	...
	assert.Never(t, func() bool {
		...
		"finalizers must not overlap store traversal")
	close(store.release)
	require.NoError(t, <-firstDone)
	require.NoError(t, <-secondDone)
	assert.Equal(t, int32(1), store.maximum.Load())
	assert.Equal(t, int32(4), store.calls.Load(),
		"the mid-run generation must produce one serialized content/metadata follow-up")
}

func TestImportDetailedFromStoreDoesNotSerializeIndependentStores(t *testing.T) {
	newBlockedStore := func(t *testing.T) *importConcurrencyStore {
		filesystem, err := newProtocolTestStore(t.TempDir())
		require.NoError(t, err)
		t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
		return &importConcurrencyStore{
			ArtifactStore: filesystem,
			entered:       make(chan struct{}),
			release:       make(chan struct{}),
		}
	}
	first := newBlockedStore(t)
	second := newBlockedStore(t)
	firstDB := testDB(t)
	secondDB := testDB(t)
	firstDone := make(chan error, 1)
	secondDone := make(chan error, 1)
	go func() {
		_, err := ImportDetailedFromStore(t.Context(), firstDB, first, "local-d4e5f6")
		firstDone <- err
	}()
	<-first.entered
	go func() {
		_, err := ImportDetailedFromStore(t.Context(), secondDB, second, "local-d4e5f6")
		secondDone <- err
	}()
	...
	require.NoError(t, <-firstDone)
	require.NoError(t, <-secondDone)
	assert.True(t, secondEntered, "independent stores must not share an import gate")
}

func TestDirectAndCoordinatorImportsSerializeForSameStore(t *testing.T) {
	filesystem, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
	store := &importConcurrencyStore{
	...
	go func() {
		_, err := ImportDetailedFromStore(t.Context(), directDB, store, "local-d4e5f6")
		directDone <- err
	}()
	<-store.entered

	coordinator := NewStoreImportCoordinator(testDB(t), store, "local-d4e5f6")
	require.NoError(t, coordinator.ScheduleArtifactImport())
	...
	assert.Never(t, func() bool {
		...
		"direct and coordinator imports must share the store-scoped gate")

	close(store.release)
	require.NoError(t, <-directDone)
	require.NoError(t, <-coordinatorDone)
	assert.Equal(t, int32(1), store.maximum.Load())
}

func TestImportDetailedFromStoreSerializesAliasDecorators(t *testing.T) {
	filesystem, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
	probe := &importConcurrencyStore{
	...
	first := &importScopeAliasStore{ArtifactStore: filesystem, probe: probe}
	second := &importScopeAliasStore{ArtifactStore: filesystem, probe: probe}
	firstDB := testDB(t)
	secondDB := testDB(t)
	firstDone := make(chan error, 1)
	secondDone := make(chan error, 1)
	...
	assert.Never(t, func() bool {
		return probe.maximum.Load() > 1
	}, 100*time.Millisecond, time.Millisecond,
		"aliases of one underlying store must share its import scope")

	close(probe.release)
	require.NoError(t, <-firstDone)
	require.NoError(t, <-secondDone)
	assert.Equal(t, int32(1), probe.maximum.Load())
}

func TestImportDetailedFromStoreDoesNotContendIndependentValueDecorators(t *testing.T) {
	newValueStore := func(t *testing.T) (valueImportStore, *importConcurrencyStore) {
		filesystem, err := newProtocolTestStore(t.TempDir())
		require.NoError(t, err)
		t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
		probe := &importConcurrencyStore{
		...
		return valueImportStore{ArtifactStore: probe}, probe
	}
	first, firstProbe := newValueStore(t)
	second, secondProbe := newValueStore(t)
	firstDone := make(chan error, 1)
	secondDone := make(chan error, 1)
	firstDB := testDB(t)
	secondDB := testDB(t)
	...
	secondEntered := false
	select {
	case <-secondProbe.entered:
		secondEntered = true
	case <-time.After(100 * time.Millisecond):
	}
	close(firstProbe.release)
	close(secondProbe.release)
	require.NoError(t, <-firstDone)
	require.NoError(t, <-secondDone)
	assert.True(t, secondEntered, "independent value decorators must not share a fallback gate")
}

func TestImportDetailedFromStoreSerializesValueFallbackForSameDatabase(t *testing.T) {
	filesystem, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
	probe := &importConcurrencyStore{
	...
	store := valueImportStore{ArtifactStore: probe}
	database := testDB(t)
	firstDone := make(chan error, 1)
	secondDone := make(chan error, 1)
	go func() {
		_, err := ImportDetailedFromStore(t.Context(), database, store, "local-d4e5f6")
		firstDone <- err
	}()
	<-probe.entered
	go func() {
	...
	assert.Never(t, func() bool {
		...
		"fallback imports into one database must not overlap")
...
func TestStoreImportCoordinatorRetriesTransientFinalizeOnceAndDrains(t *testing.T) {
	...
	assert.Equal(t, int32(3), store.calls.Load(),
		"one failed traversal and one successful content/metadata retry are expected")

	result, err = coordinator.Finalize(t.Context())
	require.NoError(t, err)
	assert.False(t, result.Changed())
	assert.Equal(t, int32(3), store.calls.Load(), "successful retry must drain pending work")
}

func TestVisitStoreOriginsJoinsCursorReleaseFailure(t *testing.T) {
	visitErr := errors.New("visit failed")
	releaseErr := errors.New("cursor release failed")
	store := &releaseFailureStore{releaseErr: releaseErr}

	err := visitStoreOrigins(t.Context(), store, "local-d4e5f6", func(string) error {
		return visitErr
	})
...
func TestImportOriginFromStorePropagatesCheckpointSpoolCleanupFailure(t *testing.T) {
	filesystem, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, filesystem.Close()) })
	cleanupErr := errors.New("checkpoint spool cleanup failed")
	originalCleanup := storeEntrySpoolCleanup
	storeEntrySpoolCleanup = func(spool *storeEntrySpool) error {
		return errors.Join(spool.close(), cleanupErr)
	}
	t.Cleanup(func() { storeEntrySpoolCleanup = originalCleanup })

	_, err = importOriginFromStore(t.Context(), testDB(t), filesystem, "peer-a1b2c3")
...
func TestImportDetailedFromStoreReplaysMetadataAfterSessionContent(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	store, err := newProtocolTestStore(t.TempDir())
...
func TestImportDetailedFromStoreQuarantinesInvalidCheckpointAndLandsOlder(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	store, err := newProtocolTestStore(t.TempDir())
	...
	require.NoError(t, err)
	ref, err := NewRef(origin, KindCheckpoints, "cp-0000000002.json")
	require.NoError(t, err)
	...
	landing, _, ok, err := target.GetArtifactCheckpointLanding(t.Context(), origin)
	require.NoError(t, err)
	require.True(t, ok)
	assert.Equal(t, 1, landing.Sequence)
}

func TestImportDetailedFromStoreQuarantinesInvalidManifestAndSegment(t *testing.T) {
	tests := []struct {
		name       string
...
func TestImportDetailedFromStoreFallsBackPastInvalidCheckpointClosure(t *testing.T) {
	tests := []struct {
		name         string
		kind         Kind
		invalidEntry func(Ref, Entry) Entry
		wantReason   string
	}{
		{
			name: "oversize manifest", kind: KindManifests,
			invalidEntry: func(ref Ref, entry Entry) Entry {
				entry.Identity.Size = manifestDecodedLimit + 1
				return entry
			},
			wantReason: "oversize manifest must not block the older closure",
		},
		{
			name: "manifest identity mismatch", kind: KindManifests,
			invalidEntry: func(ref Ref, entry Entry) Entry {
				entry.Identity.SHA256 = strings.Repeat("f", 64)
				return entry
			},
			wantReason: "manifest identity mismatch must not block the older closure",
		},
		{
			name: "oversize segment", kind: KindSegments,
			invalidEntry: func(ref Ref, entry Entry) Entry {
				entry.Identity.Size = segmentDecodedLimit + 1
				return entry
			},
			wantReason: "oversize segment must not block the older closure",
		},
		{
			name: "segment identity mismatch", kind: KindSegments,
			invalidEntry: func(ref Ref, entry Entry) Entry {
				entry.Identity.SHA256 = strings.Repeat("e", 64)
				return entry
			},
			wantReason: "segment identity mismatch must not block the older closure",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			origin := "peer-a1b2c3"
			source := testDB(t)
			seedSession(t, source, "older", "older-project")
			...
			segmentData, err := encodeSegment([]db.Message{{
				Ordinal: 0, Role: "user", Content: "newer",
			}})
			require.NoError(t, err)
			segmentHash := hashHex(segmentData)
			segmentRef, err := NewRef(origin, KindSegments, segmentHash+".ndjson")
			...
			require.NoError(t, err)
			_, err = base.Create(t.Context(), segmentRef, segmentIdentity,
				canonicalArtifactMediaType(KindSegments), bytes.NewReader(segmentData))
			require.NoError(t, err)
			m := manifest{
				Version: formatVersion, Origin: origin, NativeSessionID: "newer",
				Session:  manifestSession{ID: "newer", Machine: origin, Agent: "claude", Project: "newer-project"},
				Segments: []string{segmentHash},
			}
			manifestData, err := canonicalJSON(m)
			require.NoError(t, err)
			...
			manifestIdentity, err := NewIdentity(manifestHash, int64(len(manifestData)))
			require.NoError(t, err)
			_, err = base.Create(t.Context(), manifestRef, manifestIdentity,
				canonicalArtifactMediaType(KindManifests), bytes.NewReader(manifestData))
			require.NoError(t, err)

			invalidRef := manifestRef
			invalidEntry := Entry{Ref: manifestRef, Identity: manifestIdentity}
			if tt.kind == KindSegments {
				invalidRef = segmentRef
				invalidEntry = Entry{Ref: segmentRef, Identity: segmentIdentity}
			}
			store := &artifactEntryOverrideStore{
				ArtifactStore: base,
				ref:           invalidRef,
				entry:         tt.invalidEntry(invalidRef, invalidEntry),
			}
			newer := checkpoint{
				Version: formatVersion, Origin: origin, Sequence: 2,
				Sessions: map[string]string{origin + "~newer": manifestHash},
			}
			checkpointData, err := canonicalJSON(newer)
			require.NoError(t, err)
			checkpointRef, err := NewRef(origin, KindCheckpoints, "cp-0000000002.json")
			require.NoError(t, err)
			checkpointIdentity, err := NewIdentity(hashHex(checkpointData), int64(len(checkpointData)))
			require.NoError(t, err)
			_, err = base.Create(t.Context(), checkpointRef, checkpointIdentity,
				canonicalArtifactMediaType(KindCheckpoints), bytes.NewReader(checkpointData))
			require.NoError(t, err)

			target := testDB(t)
			result, err := ImportDetailedFromStore(t.Context(), target, store, "local-d4e5f6")
			require.NoError(t, err)
			assert.Equal(t, 1, result.Sessions, tt.wantReason)
			older, err := target.GetSession(t.Context(), origin+"~older")
			require.NoError(t, err)
			require.NotNil(t, older, tt.wantReason)
			assert.Equal(t, "older-project", older.Project)
			newerSession, err := target.GetSession(t.Context(), origin+"~newer")
			require.NoError(t, err)
			assert.Nil(t, newerSession, "invalid candidate must not be partially materialized")
			landing, _, found, err := target.GetArtifactCheckpointLanding(t.Context(), origin)
			require.NoError(t, err)
			require.True(t, found)
			assert.Equal(t, 1, landing.Sequence)
			_, err = base.Stat(t.Context(), invalidRef)
			assert.ErrorIs(t, err, ErrArtifactNotFound)
		})
	}
}

func TestImportDetailedFromStoreProcessesOriginPagesIncrementally(t *testing.T) {
	base, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, base.Close()) })
	store := &originTraversalGateStore{ArtifactStore: base}

	_, err = ImportDetailedFromStore(t.Context(), testDB(t), store, "local-d4e5f6")
	require.NoError(t, err)
	assert.True(t, store.processedFirstPage)
}

func TestImportDetailedFromStoreEnumeratesCheckpointHeadersBeforeBodies(t *testing.T) {
	origin := "peer-a1b2c3"
	base, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, base.Close()) })
...
func TestImportDetailedFromStoreNewestCheckpointSkipsHistoricalClosures(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	base, err := newProtocolTestStore(t.TempDir())
	...
	require.NoError(t, err)
	firstPage, err := base.List(t.Context(), origin, KindCheckpoints, "", 1)
	require.NoError(t, err)
	require.Len(t, firstPage.Items, 1)
	...
	result, err := ImportDetailedFromStore(t.Context(), testDB(t), store, "local-d4e5f6")
	require.NoError(t, err)
	assert.Equal(t, 1, result.Sessions)
...
func TestImportDetailedFromStoreProcessesMetadataPagesIncrementally(t *testing.T) {
	origin := "peer-a1b2c3"
	base, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, base.Close()) })
	createFutureMetadataEntries(t, base, origin, artifactImportPageSize+1)
	store := &entryTraversalGateStore{ArtifactStore: base, kind: KindMeta}

	_, err = ImportDetailedFromStore(t.Context(), testDB(t), store, "local-d4e5f6")
	require.NoError(t, err)
	assert.Equal(t, artifactImportPageSize+1, store.opens)
}

func TestImportDetailedFromStoreCancelsBetweenMetadataEntries(t *testing.T) {
	origin := "peer-a1b2c3"
	base, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, base.Close()) })
	...
	_, err = ImportDetailedFromStore(ctx, testDB(t), store, "local-d4e5f6")
	require.ErrorIs(t, err, context.Canceled)
	assert.Equal(t, 1, store.opens, "cancellation must stop before opening another event")
}

func TestImportDetailedFromStoreRejectsLongCursorCycles(t *testing.T) {
	tests := []struct {
		name         string
		cycleOrigins bool
	}{
		{name: "origins", cycleOrigins: true},
		{name: "checkpoint entries", cycleOrigins: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			base, err := newProtocolTestStore(t.TempDir())
			require.NoError(t, err)
			t.Cleanup(func() { require.NoError(t, base.Close()) })
			store := &cursorCycleStore{
				ArtifactStore: base, cycleOrigins: tt.cycleOrigins,
			}
...
func TestImportDetailedFromStoreDefersFutureManifestWithoutQuarantine(t *testing.T) {
	origin := "peer-a1b2c3"
	store, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, store.Close()) })
...
func TestImportDetailedFromStoreDefersFutureSegmentWithoutQuarantine(t *testing.T) {
	origin := "peer-a1b2c3"
	store, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, store.Close()) })
...
func TestImportDetailedFromStoreUsesOlderCheckpointBeforeFutureHead(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	store, err := newProtocolTestStore(t.TempDir())
	...
	require.NoError(t, err)
	future := checkpoint{
		Version: formatVersion + 1, Origin: origin, Sequence: 2,
	...
	assert.Equal(t, 1, result.Sessions)
	_, err = store.Stat(t.Context(), ref)
	assert.NoError(t, err, "future checkpoint must remain live")
}

func TestImportDetailedFromStoreCountsFutureCheckpointAsDeferred(t *testing.T) {
	origin := "peer-a1b2c3"
	store, err := newProtocolTestStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, store.Close()) })
...
func TestImportDetailedFromStoreLandsUpdatedLocallyTrashedSession(t *testing.T) {
	origin := "peer-a1b2c3"
	source := testDB(t)
	seedSession(t, source, "session-1", "alpha")
	store, err := newProtocolTestStore(t.TempDir())
...
type storeWithoutFolderTransport struct{ ArtifactStore }

type syncNotificationStore struct {
	ArtifactStore
	provider      ArtifactFolderTransportProvider
	notifications atomic.Int32
}

func (s *syncNotificationStore) NotifyArtifactBatch(context.Context) {
	s.notifications.Add(1)
}

func (s *syncNotificationStore) NewFolderTransport(target string) (Transport, error) {
	return s.provider.NewFolderTransport(target)
}

func TestCoordinatedTransportStoreForwardsBatchNotification(t *testing.T) {
	base := newRetentionStore(t)
	store := &syncNotificationStore{ArtifactStore: base}
	coordinator := NewStoreImportCoordinator(testDB(t), store, "local-a1b2c3")
	wrapped := newCoordinatedTransportStore(testDB(t), store, coordinator)

	notifier, ok := any(wrapped).(ArtifactBatchNotifier)
	require.True(t, ok)
	notifier.NotifyArtifactBatch(t.Context())
	assert.Equal(t, int32(1), store.notifications.Load())
}

func TestSyncWithStoreNotifiesOneCompletedBatchAcrossTransports(t *testing.T) {
	for _, suppressed := range []bool{false, true} {
		...
		for _, transportName := range []string{"folder", "http", "s3"} {
			t.Run(transportName+"/"+mode, func(t *testing.T) {
				dataDir := t.TempDir()
				repository, err := OpenRepository(t.Context(), dataDir)
				require.NoError(t, err)
				t.Cleanup(func() { require.NoError(t, repository.Close()) })
				store := &syncNotificationStore{
					ArtifactStore: repository.Store(), provider: repository,
				}
				target := notificationSyncTarget(t, transportName)
				database := testDB(t)
				...
				_, err = SyncWithStore(ctx, database, store, SyncOptions{
					DataDir: t.TempDir(), Target: target, Origin: "local-a1b2c3",
				})
				require.NoError(t, err)
				assert.Equal(t, want, store.notifications.Load())
			})
		}
	}
}

func TestSyncWithStoreFailureDoesNotNotifyCompletedBatch(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, repository.Close()) })
	store := &syncNotificationStore{ArtifactStore: repository.Store(), provider: repository}
	database := testDB(t)
	seedSession(t, database, "notify-session", "project")

	_, err = SyncWithStore(t.Context(), database, store, SyncOptions{
		DataDir: t.TempDir(), Target: peer.URL, Origin: "local-a1b2c3",
	})
	require.Error(t, err)
	assert.Zero(t, store.notifications.Load())
}

func notificationSyncTarget(t *testing.T, name string) string {
	t.Helper()
	switch name {
	case "folder":
		return t.TempDir()
	case "http":
		peer := httptest.NewServer(newFakeArtifactPeer())
		t.Cleanup(peer.Close)
		return peer.URL
	case "s3":
		backend := newMockS3(t, "bucket", 1000)
		server := httptest.NewServer(backend)
		t.Cleanup(server.Close)
		t.Setenv("AGENTSVIEW_S3_ENDPOINT", server.URL)
		t.Setenv("AGENTSVIEW_ALLOW_INSECURE_S3_ENDPOINT", "true")
		t.Setenv("AWS_ACCESS_KEY_ID", "AKIDEXAMPLE")
		t.Setenv("AWS_SECRET_ACCESS_KEY", "example-secret")
		return "s3://bucket/artifacts"
	default:
		t.Fatalf("unknown notification transport %q", name)
		return ""
	}
}

func TestSyncWithStoreFolderRequiresRetainedRootCapability(t *testing.T) {
	dataDir := t.TempDir()
	repository, err := OpenRepository(t.Context(), dataDir)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, repository.Close()) })

	_, err = SyncWithStore(t.Context(), testDB(t), storeWithoutFolderTransport{
...
func TestSyncWithStoreFolderUsesInjectedRepositoryIdentity(t *testing.T) {
	...
	t.Run("actual vault and hierarchy are rejected", func(t *testing.T) {
		...
		for name, target := range targets {
			t.Run(name, func(t *testing.T) {
				_, err := SyncWithStore(t.Context(), testDB(t), repository.Store(), SyncOptions{
...
func TestSyncFolderHealsCorruptSegmentInShare(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = bRepository.Store().Stat(ctx, segmentRef)
	assert.ErrorIs(t, err, ErrArtifactNotFound)
	...
	require.NoError(t, err)
	_, reader, err := aRepository.Store().Open(ctx, segmentRef)
	require.NoError(t, err)
