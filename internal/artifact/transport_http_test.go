...
func (s *pagedArtifactStoreObservations) ListOrigins(
	ctx context.Context, cursor Cursor, limit int,
) ([]string, Cursor, error) {
	origins, next, err := s.ArtifactStore.ListOrigins(ctx, cursor, limit)
	s.mu.Lock()
	defer s.mu.Unlock()
	s.originPages++
	s.maxRequestedEntries = max(s.maxRequestedEntries, limit)
	s.maxReturnedEntries = max(s.maxReturnedEntries, len(origins))
	return origins, next, err
}

func (s *pagedArtifactStoreObservations) List(
	...
	s.listPages[kind]++
	s.maxRequestedEntries = max(s.maxRequestedEntries, limit)
	s.maxReturnedEntries = max(s.maxReturnedEntries, len(page.Items))
	return page, err
}

func (s *pagedArtifactStoreObservations) ReleaseCursor(cursor Cursor) error {
	releaser, ok := s.ArtifactStore.(ArtifactCursorReleaser)
	if !ok {
		return errors.New("observed artifact store does not release cursors")
	}
