...
func buildTestDBTemplate() (map[string][]byte, error) {
	...
	// Checkpointing keeps the copied template compact, but it is
	// best-effort: the copy below carries the -wal/-shm files
	// along, so a checkpoint that cannot finish in time (slow
	// Windows CI runners) must not fail the build. The timeout is
	// generous because this runs once per test binary.
