...
func TestServer_SearchContentIncludeOneShot(t *testing.T) {
	...
	assert.NotContains(t, required, "include_one_shot")
	assert.NotContains(t, required, "include_automated")
...
func TestSearchContent_MultiConceptMapping(t *testing.T) {
	fake := &fakeContentSearchService{result: &service.ContentSearchResult{
		Matches: []db.ContentMatch{{
			SessionID: "shared", Ordinal: 4, OrdinalRange: [2]int{4, 6},
			Snippet: "first concept", Score: new(0.75),
			ConceptEvidence: []db.ConceptEvidence{
				{Concept: "auth model", Ordinal: 4, OrdinalRange: [2]int{4, 6}, Snippet: "auth", Score: 0.7},
				{Concept: "retry policy", Ordinal: 12, OrdinalRange: [2]int{11, 13}, Snippet: "retry", Score: 0.8},
			},
		}},
	...
	_, out, err := ts.searchContent(t.Context(), nil, searchContentIn{
		Concepts: []string{"auth model", "retry policy"}, Scope: "top", IncludeActive: true,
	})
	require.NoError(t, err)
	assert.Equal(t, []string{"auth model", "retry policy"}, fake.lastReq.Concepts)
	assert.Empty(t, fake.lastReq.Pattern)
	assert.Equal(t, "top", fake.lastReq.Scope)
	assert.Equal(t, "semantic", out.EffectiveMode)
	assert.True(t, out.CandidateTruncated)
	require.Len(t, out.Matches, 1)
	require.Len(t, out.Matches[0].ConceptEvidence, 2)
	assert.Equal(t, [2]int{11, 13}, out.Matches[0].ConceptEvidence[1].OrdinalRange)
}
