...
type toolset struct {
	...
	now func() time.Time
	// cursorKey is the per-process HMAC key that keeps body cursors
	// server-authored: a client cannot mint or edit a cursor and have the
...
type getMessagesIn struct {
	...
	BodyCursor         string   `json:"body_cursor,omitempty" jsonschema:"Opaque continuation for truncated message bodies. Use before advancing next_from; when one page truncated several messages, the cursor chains through their remainders in page order."`
}
...
// messageBodyCursor is the decoded payload of a get_messages body_cursor.
// Queue holds the page's remaining truncated messages in page order; each
// step is served only after the current body completes, so completing one
// oversized message can never skip another's unread tail.
type messageBodyCursor struct {
	...
	Queue          []bodyCursorStep `json:"q,omitempty"`
}

// bodyCursorStep pins one queued truncated message: where to resume, with
// the cursor's shared revision and evidence source defining what to read.
type bodyCursorStep struct {
	Ordinal int `json:"o"`
	Offset  int `json:"p"`
}

// newCursorKey generates the per-process HMAC key for body cursors.
...
// encodeMessageBodyCursor seals the cursor with an HMAC so any client edit
// invalidates the token.
func (t *toolset) encodeMessageBodyCursor(cursor messageBodyCursor) string {
	raw, _ := json.Marshal(cursor)
	mac := hmac.New(sha256.New, t.cursorKey)
	mac.Write(raw)
	return base64.RawURLEncoding.EncodeToString(raw) + "." +
		base64.RawURLEncoding.EncodeToString(mac.Sum(nil))
}

// decodeMessageBodyCursor verifies the HMAC before decoding, then enforces
// the field contract. Forged, tampered, and malformed tokens are the same
// invalid-cursor error.
func (t *toolset) decodeMessageBodyCursor(token string) (messageBodyCursor, error) {
	invalid := errors.New("invalid body_cursor")
	encoded, encodedMAC, ok := strings.Cut(token, ".")
	if !ok {
		return messageBodyCursor{}, invalid
	}
	...
	for _, step := range cursor.Queue {
		if step.Ordinal < 0 || step.Offset <= 0 {
			return messageBodyCursor{}, invalid
		}
	}
	return cursor, nil
...
func (t *toolset) getMessages(
	...
	t.attachBodyCursors(&out, res.EvidenceSource, in.SessionID)
	return nil, out, nil
}
...
// attachBodyCursors seals a continuation cursor onto a page's truncated
// messages. A cursor continues as a revision-bound read, so it is issued
// only when the page carries a stable transcript revision and evidence
// source; otherwise pagination stays untouched instead of minting a cursor
// the decoder would reject. Only the first truncated message carries one:
// its cursor queues the rest in page order, so continuations drain every
// oversized body before next_from pagination resumes.
func (t *toolset) attachBodyCursors(out *getMessagesOut, evidenceSource, sessionID string) {
	first := -1
	var queue []bodyCursorStep
	for i := range out.Messages {
		...
		queue = append(queue, bodyCursorStep{
			Ordinal: out.Messages[i].Ordinal,
			Offset:  utf8.RuneCountInString(out.Messages[i].Content),
		})
	}
	if first < 0 || out.TranscriptRevision == "" || evidenceSource == "" {
		return
	}
	out.Messages[first].BodyCursor = t.encodeMessageBodyCursor(messageBodyCursor{
		Version: 1, EvidenceSource: evidenceSource, SessionID: sessionID,
		Revision: out.TranscriptRevision, Ordinal: out.Messages[first].Ordinal,
		Offset:   utf8.RuneCountInString(out.Messages[first].Content),
		NextFrom: out.NextFrom, Queue: queue,
	})
	out.NextFrom = nil
}
...
func (t *toolset) getMessageBodyContinuation(
	...
	out := getMessagesOut{TranscriptRevision: cursor.Revision}
	for {
		message, err := t.readCursorMessage(ctx, cursor)
		if err != nil {
			return nil, getMessagesOut{}, err
		}
		...
		if end < len(runes) {
			cursor.Offset = end
			out.Messages[len(out.Messages)-1].BodyCursor = t.encodeMessageBodyCursor(cursor)
			return nil, out, nil
		}
		// The body is complete. Serve the next queued truncated message's
		// tail before exposing next_from, so a page with several oversized
		// messages loses none of their remainders.
		if len(cursor.Queue) == 0 {
			out.NextFrom = cursor.NextFrom
			return nil, out, nil
		}
		step := cursor.Queue[0]
		cursor = messageBodyCursor{
			Version: 1, EvidenceSource: cursor.EvidenceSource, SessionID: cursor.SessionID,
			Revision: cursor.Revision, Ordinal: step.Ordinal, Offset: step.Offset,
			NextFrom: cursor.NextFrom, Queue: cursor.Queue[1:],
		}
	}
}

// readCursorMessage fetches the one message a body-continuation step names
// and reapplies the default visibility contract. A cursor is only ever
// minted for a message the caller could already see, so a cursor naming
// system or role-filtered content is forged or stale and must not surface
// it.
func (t *toolset) readCursorMessage(
	ctx context.Context, cursor messageBodyCursor,
) (db.Message, error) {
	zero := 0
	res, err := t.svc.Messages(ctx, cursor.SessionID, service.MessageFilter{
	...
	if len(res.Messages) != 1 || res.Messages[0].Ordinal != cursor.Ordinal {
		return db.Message{}, fmt.Errorf("%w: cited message no longer exists", service.ErrSourceChanged)
	}
	message := res.Messages[0]
	if isSystemMessage(message) || !roleAllowed(message.Role, nil) {
		return db.Message{}, fmt.Errorf(
			"%w: cited message is not part of the readable conversation", service.ErrSourceChanged)
	}
	return message, nil
}
...
type searchContentIn struct {
	...
	CurrentSessionID string `json:"current_session_id,omitempty" jsonschema:"Exclude this exact full stored session ID instead of applying the recent-activity heuristic. The ID must resolve to one stored session; unknown IDs return an error instead of disabling the guard."`
	DateFrom         string `json:"date_from,omitempty" jsonschema:"Only sessions on or after this date (YYYY-MM-DD)."`
	DateTo           string `json:"date_to,omitempty" jsonschema:"Only sessions on or before this date (YYYY-MM-DD)."`
...
func (t *toolset) searchContent(
	...
	// current_session_id asserts "this is the conversation I am in" and
	// disables the recent-activity guard below. Only an ID that resolves
	// to one stored session may do that: an unknown or failing ID must
	// fail loudly, because silently keeping the guard off would expose
	// active sessions the caller did not ask about.
	if in.CurrentSessionID != "" {
		detail, err := t.svc.Get(ctx, in.CurrentSessionID)
		if err != nil {
			return nil, searchContentOut{}, fmt.Errorf("current_session_id: %w", err)
		}
		if detail == nil {
			return nil, searchContentOut{}, fmt.Errorf(
				"unknown current_session_id %q: pass the full stored session ID",
				in.CurrentSessionID)
		}
	}
