...
// collect runs rows through a reducer and returns emitted (session, ordinal).
func collect(t *testing.T, f Filter, rows []MessageInput) ([]ScopedMessage, error) {
	t.Helper()
	var out []ScopedMessage
	r := NewReducer(f, func(m ScopedMessage) { out = append(out, m) })
...
func user(session string, ord int) MessageInput {
	return MessageInput{SessionID: session, Ordinal: ord, Role: "user", HasLocalTime: true}
}

func assistant(session string, ord int, model string) MessageInput {
	return MessageInput{SessionID: session, Ordinal: ord, Role: "assistant", Model: model, HasLocalTime: true}
}

func TestReducerPairing(t *testing.T) {
	f := Filter{Models: map[string]struct{}{"sonnet": {}}}

	t.Run("selected assistant flushes preceding user", func(t *testing.T) {
		out, err := collect(t, f, []MessageInput{user("s1", 0), assistant("s1", 1, "sonnet")})
	...
	t.Run("non-selected assistant drops preceding user", func(t *testing.T) {
		out, err := collect(t, f, []MessageInput{user("s1", 0), assistant("s1", 1, "opus")})
		require.NoError(t, err)
		assert.Empty(t, out)
	})

	t.Run("session change drops pending", func(t *testing.T) {
		out, err := collect(t, f, []MessageInput{user("s1", 0), assistant("s2", 0, "sonnet")})
		require.NoError(t, err)
		require.Len(t, out, 1)
	...
	t.Run("day/hour filter drops both user and assistant", func(t *testing.T) {
		ff := Filter{Models: map[string]struct{}{"sonnet": {}}, Hour: new(9)}
	...
	// (internal/db/analytics.go); changing it is a cross-backend behavior
	// change that must update all backends and the oracle together.
...
func TestReducerOrderingError(t *testing.T) {
	f := Filter{Models: map[string]struct{}{"sonnet": {}}}

	t.Run("decreasing ordinal within session errors", func(t *testing.T) {
		_, err := collect(t, f, []MessageInput{assistant("s1", 5, "sonnet"), assistant("s1", 2, "sonnet")})
		require.Error(t, err)
	})
	...
	t.Run("non-byte-ordered session groups are accepted", func(t *testing.T) {
		out, err := collect(t, f, []MessageInput{
			user("s2", 0), assistant("s2", 1, "sonnet"),
			user("s1", 0), assistant("s1", 1, "sonnet"),
		})
		require.NoError(t, err)
		require.Len(t, out, 4)
		assert.Equal(t, "s2", out[0].SessionID)
	...
	t.Run("session reappearing after its group ends errors", func(t *testing.T) {
		_, err := collect(t, f, []MessageInput{
			assistant("s1", 0, "sonnet"),
			assistant("s2", 0, "sonnet"),
			assistant("s1", 1, "sonnet"),
		})
		require.Error(t, err)
	})

	t.Run("grouped ascending stream is fine", func(t *testing.T) {
		_, err := collect(t, f, []MessageInput{assistant("s1", 0, "sonnet"), assistant("s1", 1, "sonnet"), assistant("s2", 0, "sonnet")})
		require.NoError(t, err)
	})
}
