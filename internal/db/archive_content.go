...
func dropCopiedToolContentTx(
	...
	statements := []struct {
		...
		// Codex, TraeX, and Augure stored unpaired agent notifications as
		// ordinary user rows, with no field that distinguishes them from
		// prompts.
		{"unmarked Codex, TraeX, and Augure tool output", `
			UPDATE messages SET content = '', content_length = 0
			WHERE role = 'user' AND session_id` +
			inUnmarked(`'`+string(parser.AgentCodex)+`', '`+
				string(parser.AgentTraeX)+`', '`+
				string(parser.AgentAugure)+`'`)},
