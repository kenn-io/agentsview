...
-- Durable exact-reference work that could not finish during a bounded artifact
-- import pass. Rows are retained until compare-and-delete acknowledgement;
-- future-format work becomes eligible when the running reader advances.
CREATE TABLE IF NOT EXISTS artifact_import_queue (
    ...
    required_format_version INTEGER NOT NULL CHECK (required_format_version >= 1),
    enqueued_at             TEXT NOT NULL
        DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
...
CREATE INDEX IF NOT EXISTS idx_artifact_import_queue_pending
    ON artifact_import_queue(
        required_format_version, enqueued_at, origin, kind, name
    );
