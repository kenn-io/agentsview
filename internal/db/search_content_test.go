...
// Assistant rows before the first user message land in the exchange_no = 0
// group, which has no user anchor. Terms search must never match that group
// even when every term occurs in the preamble.
func TestSearchContentTermsIgnoresAssistantPreamble(t *testing.T) {
	d := testDB(t)
	insertSession(t, d, "terms-preamble", "proj", func(s *Session) {
		s.Agent = "claude"
		s.UserMessageCount = 1
	})
	messages := []Message{
		{
			SessionID: "terms-preamble", Ordinal: 0, Role: "assistant",
			Content: "boot gamma preamble", Timestamp: "2026-05-20T09:00:00Z",
		},
		{
			SessionID: "terms-preamble", Ordinal: 1, Role: "user",
			Content: "alpha question", Timestamp: "2026-05-20T10:00:00Z",
		},
		{
			SessionID: "terms-preamble", Ordinal: 2, Role: "assistant",
			Content: "beta answer", Timestamp: "2026-05-20T10:01:00Z",
		},
	}
	require.NoError(t, d.ReplaceSessionMessages(t.Context(), "terms-preamble", messages))

	preamble, err := d.SearchContent(t.Context(), ContentSearchFilter{
		Pattern: "gamma boot", Mode: "terms", Scope: "all", Limit: 50,
		IncludeOneShot: true,
	})
	require.NoError(t, err)
	assert.Empty(t, preamble.Matches, "assistant-only preamble must not satisfy terms queries")

	exchange, err := d.SearchContent(t.Context(), ContentSearchFilter{
		Pattern: "alpha beta", Mode: "terms", Scope: "all", Limit: 50,
		IncludeOneShot: true,
	})
	require.NoError(t, err)
	require.Len(t, exchange.Matches, 1)
	assert.Equal(t, 1, exchange.Matches[0].Ordinal)
	assert.Equal(t, "user", exchange.Matches[0].Role)
}
