...
func TestSearchContentTermsRequiresUserBoundary(t *testing.T) {
	d := testDB(t)
	insertSession(t, d, "terms-head", "proj", func(s *Session) {
		s.Agent = "claude"
		s.UserMessageCount = 1
	})
	messages := []Message{
		{
			SessionID: "terms-head", Ordinal: 0, Role: "assistant",
			Content:   "alpha beta without any user boundary",
			Timestamp: "2026-05-21T10:00:00Z",
		},
		{
			SessionID: "terms-head", Ordinal: 1, Role: "user",
			Content: "alpha", Timestamp: "2026-05-21T10:01:00Z",
		},
		{
			SessionID: "terms-head", Ordinal: 2, Role: "assistant",
			Content: "beta is set", Timestamp: "2026-05-21T10:02:00Z",
		},
	}
	require.NoError(t, d.ReplaceSessionMessages(t.Context(), "terms-head", messages))
	...
	require.NoError(t, err)
	// The assistant-only head is not an exchange; only the group anchored
	// by its user message may match.
	require.Len(t, got.Matches, 1)
	assert.Equal(t, [2]int{1, 2}, got.Matches[0].OrdinalRange)

	headOnly, err := d.SearchContent(t.Context(), ContentSearchFilter{
		Pattern: "boundary user", Mode: "terms", Scope: "all", Limit: 50,
		IncludeOneShot: true,
	})
	require.NoError(t, err)
	assert.Empty(t, headOnly.Matches)
}
