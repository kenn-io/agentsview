...
// floors, and repair work across the temp-database resync swap. Transient
// bookkeeping such as last_sync_* timestamps is deliberately left behind so
// the rebuilt DB reports its own sync times.
func (d *DB) CopySyncStateFrom(sourcePath string) error {
	...
	if oldDBHasTable(ctx, tx, "artifact_import_queue") {
		...
		if err := tx.QueryRowContext(ctx, `
			SELECT EXISTS(
				SELECT 1
				FROM main.artifact_import_queue AS current
				JOIN old_db.artifact_import_queue AS previous
				  USING (origin, kind, name)
				WHERE current.sha256 <> previous.sha256
				   OR current.size <> previous.size
			)`).Scan(&conflicting); err != nil {
			return fmt.Errorf("checking artifact import identities: %w", err)
		}
		if conflicting {
			return errors.New("copying artifact import queue found conflicting identity")
		}
	}
	artifactCopies := []struct {
		...
		},
		{
			...
			`INSERT INTO main.artifact_import_queue(
				origin, kind, name, sha256, size, reason,
			 ...
			 ON CONFLICT(origin, kind, name) DO UPDATE SET
				...
				required_format_version = max(
					artifact_import_queue.required_format_version,
					excluded.required_format_version),
				enqueued_at = min(
					artifact_import_queue.enqueued_at, excluded.enqueued_at)`,
		},
		{
		...
		},
		{
			...
			`INSERT OR REPLACE INTO main.artifact_checkpoint_landings(origin, sequence)
			 SELECT origin, sequence FROM old_db.artifact_checkpoint_landings`,
		},
		{
			"artifact_peer_checkpoint_heads",
			`INSERT OR REPLACE INTO main.artifact_peer_checkpoint_heads(
				origin, sequence, checkpoint_sha256, checkpoint_size)
			 SELECT origin, sequence, checkpoint_sha256, checkpoint_size
			 FROM old_db.artifact_peer_checkpoint_heads`,
		},
		{
			"artifact_checkpoint_landing_sessions",
			`INSERT OR REPLACE INTO main.artifact_checkpoint_landing_sessions(
				origin, gid, manifest_hash)
			 SELECT origin, gid, manifest_hash
			 FROM old_db.artifact_checkpoint_landing_sessions`,
		},
		{
			"artifact_repair_queue",
			`INSERT OR REPLACE INTO main.artifact_repair_queue(
	...
	if _, err := tx.ExecContext(ctx, `
		DELETE FROM main.artifact_import_queue AS candidate
		WHERE candidate.kind = 'checkpoints'
		  AND EXISTS (
			SELECT 1 FROM main.artifact_import_queue AS newer
			WHERE newer.origin = candidate.origin
			  AND newer.kind = 'checkpoints'
			  AND newer.name > candidate.name
		  )`); err != nil {
		return fmt.Errorf("retiring copied artifact checkpoint work: %w", err)
	}
	if err := tx.Commit(); err != nil {
...
// (metadata provenance, applied events, replay state, and conflicts) from
