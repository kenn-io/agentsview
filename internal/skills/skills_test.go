...
func TestRenderClaudeSearchAgentContract(t *testing.T) {
	...
	// allowlists exactly the two read-only AgentsView MCP tools. Inheriting
	// the parent session's other tools — including every other registered
	// MCP server — would let crafted transcript text induce privileged tool
	// calls. The allowlist pins the documented `agentsview` server name
	// (what the install docs and the native plugin's .mcp.json both use), so
	// a daemon registered under any other name fails closed: the agent
	// reports the missing tools instead of reaching for inherited ones.
	...
	assert.Contains(t, agent, "No other tool")
