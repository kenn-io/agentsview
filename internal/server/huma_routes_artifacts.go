...
func (s *Server) publishLocalArtifacts(ctx context.Context, store artifact.ArtifactStore) error {
	...
	_, recovered, err := artifact.RecoverRepositoryResetRepublish(ctx, local, store, origin)
	if err != nil {
	...
	if _, err := artifact.PublishRepositoryArtifacts(
...
type artifactCursorCycleGuard struct {
	anchor artifact.Cursor
	power  uint64
	length uint64
}

func (g *artifactCursorCycleGuard) Observe(cursor artifact.Cursor) bool {
	if cursor == "" {
		return false
	}
	if g.anchor == "" {
...
func releaseStoreCursor(store artifact.ArtifactStore, cursor *artifact.Cursor) error {
	if cursor == nil || *cursor == "" {
		return nil
	}
	var err error
	if releaser, ok := store.(artifact.ArtifactCursorReleaser); ok {
...
func latestStoreCheckpointEntry(
	...
	var cursor artifact.Cursor
	defer func() { retErr = errors.Join(retErr, releaseStoreCursor(store, &cursor)) }()
	guard := artifactCursorCycleGuard{}
	for {
		page, err := store.List(ctx, origin, artifact.KindCheckpoints, cursor, 512)
		if err != nil {
		...
		if guard.Observe(cursor) {
			return artifact.Entry{}, false, errors.New("artifact checkpoint cursor cycle")
		}
	}
}
...
func (s serverArtifactImportRetryScheduler) ScheduleArtifactImport() error {
...
func (s *Server) humaPostArtifact(
	...
	if canonicalRef.Kind == artifact.KindCheckpoints {
		var sequence int
		if _, err := fmt.Sscanf(canonicalRef.Name, "cp-%010d.json", &sequence); err != nil ||
			fmt.Sprintf("cp-%010d.json", sequence) != canonicalRef.Name {
			return nil, artifactRouteError("record peer checkpoint",
				fmt.Errorf("%w: invalid checkpoint sequence name", artifact.ErrArtifactInvalid))
		}
		if err := local.RecordArtifactPeerCheckpointHead(ctx, db.ArtifactPeerCheckpointHead{
			Origin: canonicalRef.Origin, Sequence: sequence,
			CheckpointSHA256: identity.SHA256, CheckpointSize: identity.Size,
		}); err != nil {
			return nil, artifactRouteError("record peer checkpoint", err)
		}
	}
...
func (s *Server) postArtifactExchangeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	s.lockSessionLifecycle()
	defer s.sessionLifecycleMu.Unlock()
	result, err := artifact.SyncWithStore(r.Context(), local, store, artifact.SyncOptions{
...
func (s *Server) postArtifactResetHTTP(w http.ResponseWriter, r *http.Request) {
	...
	current, ok := artifact.RepositoryFromStore(ownedStore)
	if !ok {
...
func (s *Server) postArtifactMaintenanceHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if maintainer, ok := store.(artifact.ArtifactMaintainer); ok && !input.DryRun {
		response.Physical.Supported = true
		response.Physical.Result, err = artifact.RunPhysicalMaintenance(
			r.Context(), maintainer, maintenanceOpts)
		if err != nil {
...
func (s *Server) importPeerArtifacts(
	...
	if s.metadata != nil {
		...
		res, err = s.metadata.ImportFromStore(ctx, store)
	} else {
		...
		res, err = artifact.ImportDetailedFromStore(ctx, local, store, localOrigin)
	}
	if err != nil {
