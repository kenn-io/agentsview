...
func listMetadataEntries(t *testing.T, te *testEnv) []artifact.Entry {
	...
	var origins []string
	var originCursor artifact.Cursor
	for {
		page, next, err := te.artifactStore.ListOrigins(
			t.Context(), originCursor, 64,
		)
		require.NoError(t, err)
	...
	for _, origin := range origins {
		...
		for {
			page, err := te.artifactStore.List(
				t.Context(), origin, artifact.KindMeta, cursor, 64,
			)
			require.NoError(t, err)
