...
func exportArtifactFixture(
	...
	result, err := artifact.ExportToStore(ctx, database, repository.Store(), artifact.ExportOptions{
		Origin: origin,
		Full:   true,
	})
	require.NoError(t, err)
...
func TestArtifactPeerReadRoutesServeInjectedStoreInRemoteMode(t *testing.T) {
	...
	ref := seedArtifactStore(t, repository.Store(), origin, artifact.KindRaw, raw)

	srv := server.New(cfg, artifactRemoteStore{}, nil, server.WithArtifactStore(repository.Store()))
	te := &testEnv{
		srv:     srv,
		handler: wrapTestHandler(cfg, srv.Handler()),
		dataDir: dir,
...
func TestArtifactResetDaemonRouteMovesAsideAndReportsManualRecovery(t *testing.T) {
	...
	repository, err := artifact.OpenRepository(t.Context(), dir)
	require.NoError(t, err)
	foreign := seedArtifactStore(t, repository.Store(), "peer-a1b2c3", artifact.KindRaw,
	...
	srv := server.New(cfg, database, nil, server.WithArtifactStore(repository.Store()))
	t.Cleanup(func() {
