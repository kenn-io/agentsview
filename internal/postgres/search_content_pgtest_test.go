...
// PG parity with internal/db's TestSearchContentTermsIgnoresAssistantPreamble:
// the exchange_no = 0 group (assistant rows before the first user message)
// must never match a terms query.
func TestPGSearchContentTermsIgnoresAssistantPreamble(t *testing.T) {
	store := setupContentSearch(t)
	insertCSSession(t, store, "cs-terms-preamble", "proj", "claude",
		"2026-05-01T09:00:00Z", "2026-05-01T10:30:00Z")
	insertCSMessage(t, store, "cs-terms-preamble", 0, "assistant",
		"boot gamma preamble", "2026-05-01T09:00:00Z", false)
	insertCSMessage(t, store, "cs-terms-preamble", 1, "user",
		"alpha question", "2026-05-01T10:00:00Z", false)
	insertCSMessage(t, store, "cs-terms-preamble", 2, "assistant",
		"beta answer", "2026-05-01T10:01:00Z", false)

	preamble, err := store.SearchContent(t.Context(), db.ContentSearchFilter{
	...
	exchange, err := store.SearchContent(t.Context(), db.ContentSearchFilter{
		Pattern: "alpha beta", Mode: "terms", Scope: "all", Limit: 50,
		IncludeOneShot: true,
	})
	require.NoError(t, err)
