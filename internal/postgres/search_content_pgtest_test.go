...
func TestPGSearchContentTermsSkipsAssistantPrelude(t *testing.T) {
	store := setupContentSearch(t)
	insertCSSession(t, store, "cs-prelude", "proj", "claude",
		"2026-05-21T10:00:00Z", "2026-05-21T10:30:00Z")
	insertCSMessage(t, store, "cs-prelude", 0, "user",
		"hidden setup zeta", "2026-05-21T10:00:00Z", true)
	insertCSMessage(t, store, "cs-prelude", 1, "assistant",
		"zeta appears in the prelude reply", "2026-05-21T10:01:00Z", false)
	insertCSMessage(t, store, "cs-prelude", 2, "user",
		"what is zeta", "2026-05-21T10:02:00Z", false)
	insertCSMessage(t, store, "cs-prelude", 3, "assistant",
		"zeta is configured here", "2026-05-21T10:03:00Z", false)
	...
	require.NoError(t, err)
	require.Len(t, got.Matches, 1)
	assert.Equal(t, [2]int{2, 3}, got.Matches[0].OrdinalRange,
		"an assistant prelude before the first eligible user message must not"+
			" surface as its own exchange")
}

func TestPGSearchContentTermsIgnoresUserRoleToolResults(t *testing.T) {
	store := setupContentSearch(t)
	insertCSSession(t, store, "cs-toolresult", "proj", "claude",
		"2026-05-21T11:00:00Z", "2026-05-21T11:30:00Z")
	insertCSMessage(t, store, "cs-toolresult", 0, "user",
		"show the zeta config", "2026-05-21T11:00:00Z", false)
	insertCSMessage(t, store, "cs-toolresult", 1, "assistant",
		"reading the config now", "2026-05-21T11:01:00Z", false)
	insertCSMessage(t, store, "cs-toolresult", 2, "user",
		`tool output: {"zeta": "solely_tool_output"}`,
		"2026-05-21T11:02:00Z", false)
	insertCSMessage(t, store, "cs-toolresult", 3, "assistant",
		"the zeta config is enabled", "2026-05-21T11:03:00Z", false)
	_, err := store.DB().Exec(
		`UPDATE messages SET source_subtype = 'tool_result'
		 WHERE session_id = $1 AND ordinal = 2`, "cs-toolresult")
	require.NoError(t, err)
	...
	only, err := store.SearchContent(t.Context(), db.ContentSearchFilter{
