...
func TestPushArtifactNativeAndImportedCopiesShareOriginIdentity(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = artifact.ExportToStore(ctx, originDB, artifactRepository.Store(), artifact.ExportOptions{
				Origin: originA,
				Full:   true,
			})
			require.NoError(t, err)
			...
			importResult, err := artifact.ImportDetailedFromStore(
				ctx, importerDB, artifactRepository.Store(), originB,
			)
			require.NoError(t, err)
...
func TestPushArtifactOriginAdoptionReusesLegacyBareRows(t *testing.T) {
	...
	t.Cleanup(func() { require.NoError(t, importerDB.Close()) })
	require.NoError(t, artifact.AdoptOrigin(importerDB, originB))
	artifactImportResult, err := artifact.ImportDetailedFromStore(
		ctx, importerDB, artifactRepository.Store(), originB,
	)
	require.NoError(t, err)
