...
func TestPushSessionCarriesDeletionCauseInStableParameterOrder(t *testing.T) {
	...
	err = (&Sync{machine: "push-machine"}).pushSession(
		...
		pushedSessionIdentity{ID: "session", Machine: "push-machine"},
		"marker", nil,
	)
	require.NoError(t, err)
	require.Len(t, state.upsertArgs, 64)
	...
	assert.Equal(t, "null", state.upsertArgs[63].Value)
...
func TestSessionPushFingerprintIncludesDeletionCause(t *testing.T) {
	...
	assert.NotEqual(t,
		sessionPushFingerprint(base, base.ID, base.Machine, "", "", ""),
		sessionPushFingerprint(withCause, withCause.ID, withCause.Machine, "", "", ""),
	)
}
...
var postgresParameterPattern = regexp.MustCompile(`\$(\d+)`)
...
func (c *pushSessionProbeConn) ExecContext(
	...
	case strings.Contains(normalized, "insert into sessions"):
		if err := validatePostgresParameterCount(query, len(args)); err != nil {
			return nil, err
		}
		c.state.upserts++
...
func validatePostgresParameterCount(query string, argumentCount int) error {
	highestParameter := 0
	for _, match := range postgresParameterPattern.FindAllStringSubmatch(query, -1) {
		parameter, err := strconv.Atoi(match[1])
		if err != nil {
			return fmt.Errorf("parsing postgres parameter %q: %w", match[0], err)
		}
		highestParameter = max(highestParameter, parameter)
	}
	if highestParameter != argumentCount {
		return fmt.Errorf(
			"postgres query references $%d but received %d arguments",
			highestParameter, argumentCount,
		)
	}
	return nil
}
