...
func (s *Store) searchContentTermsPG(
	...
	// Assistant rows before the first user message land in exchange_no = 0
	// groups with no user anchor; the exchange contract is user-anchored, so
	// those groups never match.
	predicates := make([]string, 0, len(terms)+1)
	predicates = append(predicates, "exchange_no > 0")
