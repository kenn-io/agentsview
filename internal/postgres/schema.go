...
func createPartialIndexesPG(ctx context.Context, db schemaExecer) error {
	indexes := []string{
		`CREATE INDEX IF NOT EXISTS idx_sessions_raw_group ON sessions(raw_group_id) WHERE raw_group_id <> ''`,
...
func ensureColumns(
	ctx context.Context, db schemaExecer,
	existing map[string]map[string]bool,
