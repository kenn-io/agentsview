...
// insertCountSession inserts a session row for CountSessionsForUsage tests.
func insertCountSession(
	...
	endedExpr := "NULL"
	if ended != "" {
		endedExpr = "'" + ended + "'::timestamptz"
	}
	deletedExpr := "NULL"
	if deleted != "" {
		deletedExpr = "'" + deleted + "'::timestamptz"
	}
	_, err := store.DB().ExecContext(ctx, `
		INSERT INTO sessions (
			...
			$1, 'test-machine', $2, $3, $4::timestamptz, `+endedExpr+`,
			$5, 1, `+deletedExpr+`
		)`, id, project, agent, started, messageCount)
	require.NoError(t, err, "insert session %s", id)
}

func TestStoreCountSessionsForUsage_NoTokenSessionCounted(t *testing.T) {
	_, store := prepareUsageSchema(t, "agentsview_count_notoken_test")
	ctx := context.Background()

	insertCountSession(t, store, "cop-1", "proj-a", "copilot",
...
func TestStoreCountSessionsForUsage_ExcludesDeletedAndEmpty(t *testing.T) {
	_, store := prepareUsageSchema(t, "agentsview_count_excl_test")
	ctx := context.Background()

	insertCountSession(t, store, "live", "proj-a", "copilot",
		"2024-06-15T10:00:00Z", "", 1, "")
	insertCountSession(t, store, "deleted", "proj-a", "copilot",
		"2024-06-15T10:00:00Z", "", 1, "2024-06-16T00:00:00Z")
	insertCountSession(t, store, "empty", "proj-a", "copilot",
...
func TestStoreCountSessionsForUsage_FilterPredicates(t *testing.T) {
	_, store := prepareUsageSchema(t, "agentsview_count_filter_test")
	ctx := context.Background()

	insertCountSession(t, store, "cop-a", "proj-a", "copilot",
		"2024-06-15T10:00:00Z", "", 1, "")
	insertCountSession(t, store, "vscop-a", "proj-a", "vscode-copilot",
		"2024-06-15T10:00:00Z", "", 1, "")
	insertCountSession(t, store, "cop-b", "proj-b", "copilot",
		"2024-06-15T10:00:00Z", "", 1, "")
	insertCountSession(t, store, "claude-a", "proj-a", "claude",
		"2024-06-15T10:00:00Z", "", 1, "")
...
func TestStoreCountSessionsForUsage_TimezoneDayBoundary(t *testing.T) {
	_, store := prepareUsageSchema(t, "agentsview_count_tz_test")
	ctx := context.Background()

	// America/New_York is UTC-4 in June (EDT).
	// local 2024-06-15 23:30 == UTC 2024-06-16 03:30 -> in-window.
	insertCountSession(t, store, "late", "proj-a", "copilot",
		"2024-06-16T03:30:00Z", "2024-06-16T03:35:00Z", 1, "")
	// local 2024-06-16 00:15 == UTC 2024-06-16 04:15 -> next local day.
	insertCountSession(t, store, "past-midnight", "proj-a", "copilot",
...
func TestStoreCountSessionsForUsage_MultiDayOverlap(t *testing.T) {
	_, store := prepareUsageSchema(t, "agentsview_count_multiday_test")
	ctx := context.Background()

	insertCountSession(t, store, "multi-day", "proj-a", "copilot",
		"2024-06-10T10:00:00Z", "2024-06-20T10:00:00Z", 1, "")
	// Null ended_at: upper endpoint falls back to started_at (in-window).
	insertCountSession(t, store, "no-end", "proj-a", "copilot",
		"2024-06-15T10:00:00Z", "", 1, "")
	insertCountSession(t, store, "before", "proj-a", "copilot",
