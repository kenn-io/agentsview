...
// SuppressShadowedOpenRouterRows exists to protect. A bare session
// model name resolves against a lone provider-qualified key, but two
// qualified keys under the same provider that canonicalize alike tie
// within one rank and stay unresolved. Merging two catalogs that spell
// the same model differently (LiteLLM's minimax/MiniMax-M3 against
// OpenRouter's minimax/minimax-m3) would otherwise leave the model
// unpriced instead of using the higher-priority rate.
