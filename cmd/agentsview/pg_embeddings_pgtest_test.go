...
`, ownerURL, cfg.PG.Schema, cfg.PG.RawTenant, cfg.PG.URL, cfg.PG.Schema, cfg.PG.RawTenant, endpoint.URL)
	...
	assert.Contains(t, output, "activation_mode=automatic")
	...
	_, err = run("provision", "owner", "--profile", "current", "--runtime-role", runtimeRole, "--instance-key", "initial", "--activation-mode", "manual")
	require.ErrorContains(t, err, "activation mode")
	...
	output, err = run("rebuild", "owner", "--profile", "current", "--runtime-role", runtimeRole, "--instance-key", "after-provider-fix", "--activation-mode", "manual")
	require.NoError(t, err)
	assert.Contains(t, output, "Generation 2", "a fresh rebuild key must select a fresh generation")
	assert.Contains(t, output, "activation_mode=manual")
	...
	assert.Equal(t, "manual", desiredStatus["activation_mode"])
