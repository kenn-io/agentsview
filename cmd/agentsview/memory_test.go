...
// An environment-selected hosted-reader server must never receive
// credentials: project settings can inject the endpoint, and both the
// global token fallback and an environment-sourced token file would
// disclose archive credentials to whoever injected it.
func TestMemorySessionStartEnvServerIgnoresEnvTokenFile(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("USERPROFILE", t.TempDir())
	t.Setenv("AGENTSVIEW_DATA_DIR", t.TempDir())
	original := runMemorySessionStart
	...
	t.Setenv("AGENTSVIEW_MEMORY_SERVER", "https://reader.example")
	t.Setenv("AGENTSVIEW_SERVER_TOKEN", "global-secret")
	tokenFile := filepath.Join(t.TempDir(), "token")
	require.NoError(t, os.WriteFile(tokenFile, []byte("file-secret\n"), 0o600))
	t.Setenv("AGENTSVIEW_MEMORY_SERVER_TOKEN_FILE", tokenFile)
	...
	_, err := executeCommand(newRootCommand(), "memory", "session-start")
	require.NoError(t, err)
	assert.Empty(t, got.ServerToken,
		"an env-selected server must not receive an environment-sourced token")
}

// Without an explicit token file, an environment-selected server is probed
// unauthenticated: the global token must not be sent to an endpoint the
// environment could have redirected.
func TestMemorySessionStartEnvServerNeverUsesGlobalToken(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("USERPROFILE", t.TempDir())
	t.Setenv("AGENTSVIEW_DATA_DIR", t.TempDir())
	original := runMemorySessionStart
	...
	require.NoError(t, err)
	assert.Empty(t, got.ServerToken,
		"the global token must not be sent to an environment-selected server")
}

// A user-typed --server flag keeps the documented global-token fallback:
// explicit arguments are trusted, injected environment is not.
func TestMemorySessionStartExplicitServerKeepsGlobalTokenFallback(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("USERPROFILE", t.TempDir())
	t.Setenv("AGENTSVIEW_DATA_DIR", t.TempDir())
	original := runMemorySessionStart
	...
	_, err := executeCommand(newRootCommand(), "memory", "session-start",
		"--mode", "hosted-reader", "--server", "https://reader.example")
	require.NoError(t, err)
	assert.Equal(t, "global-secret", got.ServerToken,
		"an explicit server keeps the documented global-token fallback")
}
...
// Registration creates the default data directory when it does not exist
// yet, so a fresh install can record the hook binary on first use.
func TestMemorySessionStartRegistersBinaryCreatingDataDir(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("USERPROFILE", t.TempDir())
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	_, err := executeCommand(newRootCommand(), "memory", "session-start")
	require.NoError(t, err)
	record, readErr := os.ReadFile(
		filepath.Join(home, ".agentsview", "registered-bin"))
	require.NoError(t, readErr, "the record must be created with its parent directory")
	assert.NotEmpty(t, string(record))
}
...
// A request arriving while refresh runs may describe work the pass did not
// scan (it landed after the source was processed), so the scheduler must run
// another debounced pass for it instead of dropping the buffered signal.
func TestMemoryRefreshSchedulerRerunsForRequestDuringRefresh(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		ctx, cancel := context.WithCancel(t.Context())
		defer cancel()
		queue := newMemoryRefreshQueue()
		...
		go runMemoryRefreshScheduler(ctx, queue.requests, time.Second, func() {
			if runs.Add(1) == 1 {
				// Simulate a SessionStart hook signaling mid-refresh: too
				// late for this pass's source scan to have covered it.
				queue.Notify()
			}
		})
		...
		assert.Equal(t, int32(2), runs.Load(),
			"a request arriving during refresh must trigger a second pass")
	})
}
