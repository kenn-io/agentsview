...
func TestMemoryRefreshSchedulerRerunsForRequestArrivingDuringRefresh(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		ctx, cancel := context.WithCancel(t.Context())
		defer cancel()
		queue := newMemoryRefreshQueue()
		...
		time.Sleep(time.Second)
		<-firstStarted
		// A notification naming a provider the running pass may have already
		// scanned arrives mid-refresh. The scheduler cannot tell whether the
		// pass covered it, so it must run a confirming pass instead of
		// dropping the signal.
		queue.Notify()
		close(releaseFirst)
