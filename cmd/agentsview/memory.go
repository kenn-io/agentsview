...
func runMemoryRefreshScheduler(
	...
	debounceLoop:
		...
		refresh()
		// A request that arrived while refresh ran may target a provider this
		// pass already scanned, but the scheduler cannot tell which. Leave any
		// queued signal in place so the next iteration runs a confirming pass
		// instead of assuming the burst was covered; the bounded queue still
		// collapses a parallel SessionStart burst into that one extra pass.
	}
}
